...
%%% Phase 2
elseif time > ( t0 + minus_time - d_time )  &&  time < ( t0 + minus_time + d_time )    % なぜかPhase2に入らないという現象のため，==ではなく不等式にした
Phase = 2;
...
% 
% else
%    theta_com = abs( atan2( ( ten3(2,1) - ten2(2,1) ), ( ten3(1,1) - ten2(1,1) ) ) );  % ターゲット姿勢
   theta_chohou = abs( atan( half_side_cho / half_side_tan ) );
...
%   kakuR = kakudo( [ POS_j7' 0 ]', [ POS_j6_st' 0 ]', ten4 );
  ...
  d_POS_j4 = d_geo - D_0 * [ 1 0 0 ]';   % 目標手先位置
...
%   end
   ...
   theta_com = atan2( ( ten3(2,1) - ten2(2,1) ), ( ten3(1,1) - ten2(1,1) ) );  % ターゲット姿勢
%   theta_chohou = abs( atan( half_side_cho / half_side_tan ) );
