...
= Chohou_FourTips_RIGHT_CONTACT_v01( SV_d, SV_tc, d_time, r_tip, contact_flag_R, contactflag_R, shikaku1, shikaku2, shikaku3, shikaku4, ...
                                     l_side_tan_min, l_side_cho_min, l_corner_min, tc_geo, q0, Rg_d, POS_eR, delta_tmp_R, curPosAP3_tmp_R, ...
                                     l1, l2, l3, l4, g1, g2, g3, g4, cof_R, kkk, ccc, half_side_tan, half_side_cho )
...
if     contact_flag_R(1,1) > 0  ||  contact_flag_R(2,2) > 0  ||  contact_flag_R(3,3) > 0  ||  contact_flag_R(4,4) > 0
   ...
   r_R = norm( shikaku_R - POS_eR );   % ターゲット中心から手先球中心までの距離
   l_x_R =  0.0;
% めり込み量設定
...
% 接触面角度
    if shikaku_R(1,1) == POS_eR(1,1)   % 手先球中心とターゲット中心が同じx座標のとき
      if shikaku_R(2,1) > POS_eR(2,1)   % 手先球の真上にターゲットがある場合
          ...
          norm_R = [ cos(theta_R_yaw) sin(theta_R_yaw) 0 ]';   %法線単位ベクトル(ターゲット外側向き，手先が受ける法線力の向き) [0 -1 0]
      else                                 % 手先球の真下にターゲットがある場合
          theta_R_yaw = pi/2;   % 右に右手先，左にターゲットがある状態を角度0として，接触面はπ/2
          norm_R = [ cos(theta_R_yaw) sin(theta_R_yaw) 0 ]';   % [0 1 0]
      end
    else   % 手先球中心とターゲット中心が異なるx座標とき
          theta_R_yaw = atan2( ( shikaku_R(2,1) - POS_eR(2,1) ), ( shikaku_R(1,1) - POS_eR(1,1) ) );   % 逆正接→角度導出  θ1'
          norm_R = [ cos(theta_R_yaw-pi) sin(theta_R_yaw-pi) 0 ]';
    end
...
% ターゲット重心座標系から見た接触位置
    curPosAP3_R = curPosAP3_R - SV_tc.R0;   % APを使って慣性座標中心をターゲット重心座標中心に替える
    TB0_R = rpy2dc( SV_tc.Q0 );   % ターゲット回転角の方向余弦行列を作る
...
% 接触力の計算
  if delta_R >= 0.0  ||  r_tip - r_R >= 0.0  ||  l_corner_min >= ( sqrt( ( tc_geo(1,1) - POS_eR(1,1) )^2 + ( tc_geo(2,1) - POS_eR(2,1) )^2 ) - r_tip )  % めり込み量が正のとき(接触した瞬間からめり込んでいるとき)
      contactflag_R = 1;
...
% 摩擦力の計算
  if delta_R >= 0.0  ||  r_tip - r_R >= 0.0  ||  l_corner_min >= ( sqrt( ( tc_geo(1,1) - POS_eR(1,1) )^2 + ( tc_geo(2,1) - POS_eR(2,1) )^2 ) - r_tip )

    if     contact_flag_R(1,1) > 0
        psi_R = abs( pi/2 - abs( q0 + g_R - theta_R_yaw ) );
        corner_vel_R = l_tcR0 * SV_tc.w0(3,1);   % 四角の頂点の速度(ターゲット重心から頂点までの距離×ターゲットの角速度)
    elseif contact_flag_R(2,2) > 0
        psi_R = abs( q0 + g_R - theta_R_yaw );
        corner_vel_R = l_tcR0 * SV_tc.w0(3,1);   % 四角の頂点の速度(ターゲット重心から頂点までの距離×ターゲットの角速度)
    elseif contact_flag_R(3,3) > 0
        psi_R = abs( pi/2 - abs( q0 + g_R - theta_R_yaw ) );
        corner_vel_R = l_tcR0 * SV_tc.w0(3,1);   % 四角の頂点の速度(ターゲット重心から頂点までの距離×ターゲットの角速度)
   ...
   elseif POS_eR(1,1) > SV_tc.R0(1,1)
...
%%%%%%%% 手先が領域12,23,34,41にあるとき(面接触) %%%%%%%%

elseif contact_flag_R(1,2) > 0  ||  contact_flag_R(2,3) > 0  ||  contact_flag_R(3,4) > 0  ||  contact_flag_R(4,1) > 0


if     ( contact_flag_R(1,2) > 0 && ( 0 <= q0 && q0 < pi/2 ) ) || ( contact_flag_R(4,1) > 0 && ( pi/2 <= q0 && q0 < pi ) )
   if contact_flag_R(4,1) > 0
      q0 = q0 - pi/2;
      l_side_min = l_side_cho_min;
      half_side = half_side_cho;
   else
      l_side_min = l_side_tan_min;
      half_side = half_side_tan;
   end

   l_R = norm( tc_geo - POS_eR );   % ターゲット中心から右手先球中心までの距離 正
    if     0 <= q0  &&  q0 < pi/4   % この分岐内容が接触領域によって変わります
        if     POS_eR(2,1) > tc_geo(2,1)
            phi_R = abs( acos( abs( POS_eR(1,1) - tc_geo(1,1) ) / l_R ) + q0 );   % ターゲット中心位置と手先中心位置を結ぶ斜辺と，ターゲット中心を通りターゲット四角形の上下辺に平行な直線が作る底辺を用いた，直角三角形の鋭角φ
        elseif POS_eR(2,1) < tc_geo(2,1)
            phi_R = abs( acos( abs( POS_eR(1,1) - tc_geo(1,1) ) / l_R ) - q0 );
        else
            phi_R = q0;
        end
    elseif pi/4 < q0  &&  q0 < pi/2
        if     POS_eR(1,1) < tc_geo(1,1)
            phi_R = abs( acos( abs( POS_eR(1,1) - tc_geo(1,1) ) / l_R ) - q0 );
        elseif POS_eR(1,1) > tc_geo(1,1)
            phi_R = abs( pi - acos( abs( POS_eR(1,1) - tc_geo(1,1) ) / l_R ) - q0 );
        else
            phi_R = abs( pi/2 - q0 );
        end
    else   % q0 = π/4のとき
            phi_R = abs( acos( abs( POS_eR(1,1) - tc_geo(1,1) ) / l_R ) - pi/4 );
    end
...
elseif ( contact_flag_R(2,3) > 0 && ( 0 <= q0 && q0 < pi/2 ) ) || ( contact_flag_R(1,2) > 0 && ( pi/2 <= q0 && q0 < pi ) )
   ...
   l_R = norm( tc_geo - POS_eR );   % ターゲット中心から右手先球中心までの距離 正
    if     0 <= q0  &&  q0 < pi/4   % この分岐内容が接触領域によって変わります
        if     POS_eR(1,1) < tc_geo(1,1)
            phi_R = abs( acos( abs( POS_eR(2,1) - tc_geo(2,1) ) / l_R ) + q0 );   % ターゲット中心位置と手先中心位置を結ぶ斜辺と，ターゲット中心を通りターゲット四角形の上下辺に平行な直線が作る底辺を用いた，直角三角形の鋭角φ
        elseif POS_eR(1,1) > tc_geo(1,1)
            phi_R = abs( acos( abs( POS_eR(2,1) - tc_geo(2,1) ) / l_R ) - q0 );
        else
            phi_R = q0;
        end
    elseif pi/4 < q0  &&  q0 < pi/2
        if     POS_eR(2,1) < tc_geo(2,1)
            phi_R = abs( acos( abs( POS_eR(2,1) - tc_geo(2,1) ) / l_R ) - q0 );
        elseif POS_eR(2,1) > tc_geo(2,1)
            phi_R = abs( pi - acos( abs( POS_eR(2,1) - tc_geo(2,1) ) / l_R ) - q0 );
        else
            phi_R = abs( pi/2 - q0 );
        end
    else   % q0 = π/4のとき
            phi_R = abs( acos( abs( POS_eR(2,1) - tc_geo(2,1) ) / l_R ) - pi/4 );
    end
...
elseif ( contact_flag_R(3,4) > 0 && ( 0 <= q0 && q0 < pi/2 ) ) || ( contact_flag_R(2,3) > 0 && ( pi/2 <= q0 && q0 < pi ) )
   if contact_flag_R(2,3) > 0
      q0 = q0 - pi/2;
      l_side_min = l_side_cho_min;
      half_side = half_side_cho;
   else
...
elseif ( contact_flag_R(4,1) > 0 && ( 0 <= q0 && q0 < pi/2 ) ) || ( contact_flag_R(3,4) > 0 && ( pi/2 <= q0 && q0 < pi ) )
   if contact_flag_R(3,4) > 0
      q0 = q0 - pi/2;
      l_side_min = l_side_tan_min;
      half_side = half_side_tan;
   else
