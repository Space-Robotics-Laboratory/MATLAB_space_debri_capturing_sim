...
paramatrix = { ...
...'_dt=0.001,et=20,Xtc=0,Ytc=0,Vxtc=0,Vytc=0,THtc=0,Wtc=1,ErL=0,ErTH=45';
'_dt=0.001,et=20,Xtc=0,Ytc=0,Vxtc=0,Vytc=0,THtc=0,Wtc=1,ErL=0,ErTH=45';
};

for iii = 1:1:length(daymatrix)
daym = char(daymatrix(iii));
parm = char(paramatrix(iii));
...
path = 'dt=0.001,et=20,Xtc=0,Ytc=0,Vxtc=0,Vytc=0,THtc=0,Wtc=1,ErL=0,ErTH=45';
% 時間指定
timepath = [ day, '-', '194319' ] ;
% パス指定
...
datepath2 = [ 'C:\Users\falsi\Desktop\最終審査\figs\v2\SimC\jushin=kikachushin/', daym, parm ];
datpath2 = [ datepath2, '/', daym, '-', 'dat' ];
movpath2 = [ datepath2, '/', daym, '-', 'mov' ];
...
% 機構長さ
mat_01 = dlmread( [ datpath2, '/', daym, '_', '01_Length_etc.dat' ], '\t', 1, 0 );
% 推定情報
mat_11 = dlmread( [ datpath2, '/', daym, '_', '11_Estimation.dat' ], '\t', 1, 0 );
% 力 アニメーション用
mat_14 = dlmread( [ datpath2, '/', daym, '_', '14_Force_ForMovie.dat' ], '\t', 1, 0 );
% ロボット アニメーション用
mat_15 = dlmread( [ datpath2, '/', daym, '_', '15_Robot_ForMovie.dat' ], '\t', 1, 0 ); 
% ターゲット アニメーション用
mat_16 = dlmread( [ datpath2, '/', daym, '_', '16_CTarget_ForMovie.dat' ], '\t', 1, 0 );
% memo
mat_17 = dlmread( [ datpath2, '/', daym, '_', '17_memo.dat' ], '\t', 1, 0 ); 
...
for_d_angle2 = linspace( 0, pi / 2 , n );
...
elseif kakudaimode == 1
 ...
 if mannakamode == 1
 xlim( [ pos_tc_geo(1,1)-mannaka, pos_tc_geo(1,1)+mannaka ] ); ylim( [ pos_tc_geo(1,2)-mannaka, pos_tc_geo(1,2)+mannaka ] );
 end
...
%   if gridmode == 1
%      moviename = [ movpath2, '/', timepath, '_DualArm_TargetChohou_', 'FrameRate', num2str(framerate), '_MovSimpleKakudaiGrid.avi' ];
%   else
%      moviename = [ movpath2, '/', timepath, '_DualArm_TargetChohou_', 'FrameRate', num2str(framerate), '_MovSimpleKakudai.avi' ];
%   end
% else
%      moviename = [ movpath2, '/', timepath, '_DualArm_TargetChohou_', 'FrameRate', num2str(framerate), '_Mov.avi' ];
% end
% 
% moviename2 = [ movpath2, '/', daym, '_DualArm_TargetChohou_', 'FrameRate', num2str(framerate), '_Mov2.avi' ];
moviename = [ movpath2, '/', daym, '_DualArm_TargetChohou_', 'FrameRate', num2str(framerate), '_Mov.avi' ];
if mannakamode == 1
moviename = [ movpath2, '/', daym, '_DualArm_TargetChohou_', 'FrameRate', num2str(framerate), '_MovOC.avi' ];
end
...
% ロボット
    ...
    % クロージャ
l_target = sqrt( half_side_tan^2 + half_side_cho^2 );
theta_c = abs( atan( half_side_cho/half_side_tan ) );
 if mannakamode == 1
I = 1;
s11 = pos_eL1 + l_target * [ cos(pi-theta_c + ang_tr ), sin(pi-theta_c + ang_tr ) ];
s21 = pos_eL1 + l_target * [ cos(theta_c-pi + ang_tr ), sin(theta_c-pi + ang_tr ) ];
s31 = pos_eL1 + l_target * [ cos(-theta_c + ang_tr ), sin(-theta_c + ang_tr ) ];
s41 = pos_eL1 + l_target * [ cos(theta_c + ang_tr ), sin(theta_c + ang_tr ) ];
s12 = pos_eL2 + l_target * [ cos(pi-theta_c + ang_tr ), sin(pi-theta_c + ang_tr ) ];
s22 = pos_eL2 + l_target * [ cos(theta_c-pi + ang_tr ), sin(theta_c-pi + ang_tr ) ];
s32 = pos_eL2 + l_target * [ cos(-theta_c + ang_tr ), sin(-theta_c + ang_tr ) ];
s42 = pos_eL2 + l_target * [ cos(theta_c + ang_tr ), sin(theta_c + ang_tr ) ];
s13 = pos_eR1 + l_target * [ cos(pi-theta_c + ang_tr ), sin(pi-theta_c + ang_tr ) ];
s23 = pos_eR1 + l_target * [ cos(theta_c-pi + ang_tr ), sin(theta_c-pi + ang_tr ) ];
s33 = pos_eR1 + l_target * [ cos(-theta_c + ang_tr ), sin(-theta_c + ang_tr ) ];
s43 = pos_eR1 + l_target * [ cos(theta_c + ang_tr ), sin(theta_c + ang_tr ) ];
s14 = pos_eR2 + l_target * [ cos(pi-theta_c + ang_tr ), sin(pi-theta_c + ang_tr ) ];
s24 = pos_eR2 + l_target * [ cos(theta_c-pi + ang_tr ), sin(theta_c-pi + ang_tr ) ];
s34 = pos_eR2 + l_target * [ cos(-theta_c + ang_tr ), sin(-theta_c + ang_tr ) ];
s44 = pos_eR2 + l_target * [ cos(theta_c + ang_tr ), sin(theta_c + ang_tr ) ];
...
S4 = fill( [ s14(I,1)+r_tip*cos(pi/2+ang_tr(I,1)), s14(I,1)+r_tip*cos(pi/2+ang_tr(I,1)+for_d_angle2), s14(I,1)+r_tip*cos(pi+ang_tr(I,1)), ...
    ...
    tg_geo = fill( pos_tc_geo(1,1) + r0c/5, pos_tc_geo(1,2) + r0s/5, 'y-', 'LineWidth', sen2 );

if simplemode == 0
    % 推定情報
    d_rb_j4 = fill( d_pos_j4(1,1) + r2s, d_pos_j4(1,2) + r2c, orange, 'LineWidth', sen1 ); d_rb_j8 = fill( d_pos_j8(1,1) + r2s, d_pos_j8(1,2) + r2c, 'b', 'LineWidth', sen1 );
...
if simplemode == 0
     ...
     % クロージャ 
       ...
       set( rb_tip_L1, 'XData', pos_eL1(i,1) + r0s, 'YData', pos_eL1(i,2) + r0c ); set( rb_tip_L2, 'XData', pos_eL2(i,1) + r0s, 'YData', pos_eL2(i,2) + r0c );
       set( rb_tip_R1, 'XData', pos_eR1(i,1) + r0s, 'YData', pos_eR1(i,2) + r0c ); set( rb_tip_R2, 'XData', pos_eR2(i,1) + r0s, 'YData', pos_eR2(i,2) + r0c );
       set( tg_geo, 'XData', pos_tc_geo(i,1) + r0c/5, 'YData', pos_tc_geo(i,2) + r0s/5 );
...
if simplemode == 0
     % 推定情報
       ...
       if mannakamode == 1
          xlim( [ pos_tc_geo(i,1)-mannaka, pos_tc_geo(i,1)+mannaka ] ); ylim( [ pos_tc_geo(i,2)-mannaka, pos_tc_geo(i,2)+mannaka ] );
       end
    ...
    i = i + 1;
%{
    if rem( mvtime(i), movie_d_time * 10 ) == 0  && mvtime(i) <= 2.8   % mvtime(i)をmovie_d_timeで除算した後の剰余が0のとき(割り切れるとき)
        % 図を保存
        figure(n_figure)
        title( sprintf( 'Time = %0.3f [s]', mvtime(i,1) ), 'FontName', 'Times New Roman', 'FontSize', FST );
