...
%シミュレーション条件
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Param.EndTime   = 0.5;    %シミュレーション終了時間[s]
...
Param.LaLength = 0.250;
Param.LbLength = 0.180;
Param.LcLength = 0.0950;
Param.LdH = 0.05;
...
Param.J1Distance = [-Param.BaseWidth/2,  Param.BaseDepth/2, 0]';
Param.J5Distance = [ Param.BaseWidth/2,  Param.BaseDepth/2, 0]';

%リンクi重心から関節jまでの距離をParam.Center2Joint(:,i,j)で表現する．座標系はリンク根元座標で表現
...
Param.Center2Joint = zeros(3,8,8);             %初期化
...
Param.Center2Joint(:,5,6) = [ 0  0.00459 0 ]'; % リンク5の重心から先端側の関節(J6)への位置ベクトル
Param.Center2Joint(:,6,6) = [ 0 -0.17641 0 ]'; % リンク6の重心から根本側の関節(J6)への位置ベクトル
Param.Center2Joint(:,6,7) = [ 0  0.00459 0 ]'; % リンク6の重心から先端側の関節(J7)への位置ベクトル
Param.Center2Joint(:,7,7) = [ 0 -0.03    0 ]'; % リンク7の重心から根本側の関節(J7)への位置ベクトル
Param.Center2Joint(:,7,8) = [ 0  0.02    0 ]'; % リンク7の重心から先端側の関節(J8)への位置ベクトル
...
Param.LinkAngLeft  =  [pi/3 -pi*4/9 -pi*7/18 0]';  % 左手の関節角度，ベース側から ;SV_q
Param.LinkAngRight = -[pi/3 -pi*4/9 -pi*7/18 0]';  % 右手の関節角度，ベース側から ;SV_q
