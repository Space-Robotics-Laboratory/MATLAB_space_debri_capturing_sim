...
zeta = [ d_time, 2.5, Theta_h_i, ts_X_i, ts_Y_i, ts_W_i, Vh_ini_i, Kh_i, ts_Deg_i, ts_Error_Length_i, ts_Error_Theta_i, ts_Vx_i, ts_Vy_i ]';
...
delta_t_2 = 0.6; %Phase6における手先速度制御時間
delta_t_3 = 0.6;

     t1 = t0 + minus_time + delta_t; %固定値
     minus_time_2 = t1 + 1.2; %固定値
%{
...
%       QR_6 = dc2rpy( oR(:,end-5:end-3)' );
    ...
    x_gh_L = JsL * Hs' * [ P_d' L_d']' ;
                       
    x_gh_R = JsR * Hs' * [ P_d' L_d']' ;
    
    x_gh = [x_gh_L' x_gh_R']' ;
    x_gh_d = ( x_gh - x_gh_tmp ) / d_time ;
...
= DesiredHandMotion_v05_InC( d_time, time, minus_time, minus_time_2, minus_time_3, SV_d, SV_ts, Obs1, Obs2, est_P, est_V, est_Q, est_q, est_W, est_geo, ...
...
Kd_wrist = 0.03;
%{
M_total = [  LP_d.mass * eye( 3, 3 )      zeros( 3, 3 )                zeros( 3, 3 )               zeros( 3, 3 );
              zeros( 3, 3 )               LP_d.inertia0                zeros( 3, 3 )               zeros( 3, 3 );
              zeros( 3, 3 )               zeros( 3, 3 )                LP_d.mass * eye( 3, 3 )     zeros( 3, 3 );
              zeros( 3, 3 )               zeros( 3, 3 )                zeros( 3, 3 )               LP_d.inertia0];
      %}    

F_tip_total = [Fh_L' Fh_R']';
...
%}
        ...
        SV_tau =  H_asuta * Je_3_G * ...
        ( ( F_tip_total ...
        + Di * ( [ zeros(1,5), ve_des(6,1)', zeros(1,5), ve_des(12,1)' ]' - [ zeros(1,5), qd_joints(3,1), zeros(1,5), qd_joints(7,1) ]' ) ...
        + Ki * ( [ zeros(1,5), xe_des(6,1)', zeros(1,5), xe_des(12,1)' ]' - [ zeros(1,5),  q_joints(3,1), zeros(1,5),  q_joints(7,1) ]' ) ...
        + Di * ( [ ve_des(1:3,1)', zeros(1,3), ve_des(7:9,1)', zeros(1,3) ]' - [ VEL_j4', zeros(1,4), VEL_j8', zeros(1,4) ]' ) ...
        + Ki * ( [ xe_des(1:3,1)', zeros(1,3), xe_des(7:9,1)', zeros(1,3) ]' - [ POS_j4', zeros(1,4), POS_j8', zeros(1,4) ]' ) ) / Mi ...
        - Jd_3_G * qd_joints ) ... - x_gh_d ) ...
        - J_3_G' * F_tip_total + C_asuta ...
        + Kd_wrist * ( zeros(8,1) - qd_wrist ) + Kp_wrist * ( zeros(8,1) - q_wrist );
 ...
 ...             + 3 * ( [ q_joints_kotei(1:3)', 0, q_joints_kotei(5:7)', 0 ]' - [ q_joints(1:3)', 0, q_joints(5:7)', 0 ]' ) ...
          ...
          if ( (isequal(F_tip_total(1:6,1), zeros(6,1)) ) == 1 && ( isequal(F_tip_total(7:12,1), zeros(6,1)) ) == 0 )
          
              SV_d.tau( 5:8, 1 ) = SV_tau( 5:8, 1 ) ;
          
          elseif( (isequal(F_tip_total(7:12,1), zeros(6,1)) ) == 1 && ( isequal(F_tip_total(1:6,1), zeros(6,1)) ) == 0 )
...
Kd_wrist = 0.03;
 ...
 KL = kakudo( [ POS_j3' 0 ]', [ ( 2 * POS_j3 - POS_j2 )' 0 ]', SV_ts.R0 );
 KR = kakudo( [ POS_j7' 0 ]', [ ( 2 * POS_j7 - POS_j6 )' 0 ]', SV_ts.R0 );
     SV_d.tau = H_asuta * qdd_2 + C_asuta - JL' * Fh_L - JR' * Fh_R ... 
 ...             + 1 * ( zeros(8,1) - [ qd_joints(1:3)', 0, qd_joints(5:7)', 0 ]' ) ...
 ...             + 3 * ( [ q_joints_kotei(1:3)', 0, q_joints_kotei(5:7)', 0 ]' - [ q_joints(1:3)', 0, q_joints(5:7)', 0 ]' ) ...
              + 1 * ( zeros(8,1) - [ qd_joints(1:3)', 0, qd_joints(5:7)', 0 ]' ) ...
              + 3 * ( [ q_joints_kotei(1:2)', KL, 0, q_joints_kotei(5:6)', KR, 0 ]' ...
...
% {
    if rem( mvtime(i), movie_d_time * 10 ) == 0 && mvtime(i) >= 0 && mvtime(i) <= 10    % mvtime(i)をmovie_d_timeで除算した後の剰余が0のとき(割り切れるとき)
        % 図を保存
        figure(n_figure)
