...
path = 'dt=0.001,et=20,Xtm=0,Ytm=0,Vxtm=0,Vytm=0,THtm=0,Wtm=1,ErL=0.25,ErTH=45';


'2019-0128-131241_dt=0.001,et=20,Xtm=0,Ytm=0,Vxtm=0,Vytm=0,THtm=0,Wtm=1,ErL=0.25,ErTH=45';
...
figpath = [ datepath, path, '/', timepath, '-', 'fig' ];
pngpath = [ datepath, path, '/', timepath, '-', 'png' ];
...
% ターゲット重心 運動
mat_05 = dlmread( [ datpath, '/', timepath, '_', '05_MTargetCoM_Motion.dat' ], '\t', 1, 0 );
% ターゲット重心 運動
mat_06 = dlmread( [ datpath, '/', timepath, '_', '06_MTargetEE_Motion.dat' ], '\t', 1, 0 );
% ロボット 力学
mat_07 = dlmread( [ datpath, '/', timepath, '_', '07_Robot_Kinetics.dat' ], '\t', 1, 0 );
% ターゲット 力学
mat_08 = dlmread( [ datpath, '/', timepath, '_', '08_MTarget_Kinetics.dat' ], '\t', 1, 0 );
% 接触フラグ
...
d_t = mat_01(1,2);
r_t = d_t / 2;
l = mat_01(1,16)*sin(mat_01(1,17))*2;  % 円の場合，L_hiraki = 0.13233
L1 = mat_15(:,21:22);
L2 = mat_15(:,24:25);
R1 = mat_15(:,27:28);
R2 = mat_15(:,30:31);
...
ll(i,1) = l;
Rd(i,1) = d_t+d;%ターゲット直径

Maxa(i,1) = Rd(i,1);
Maxb(i,1) = sqrt( Rd(i,1)^2 + ll(i,1)^2 )*sqrt(2);

Mina(i,1) = sqrt( Rd(i,1)^2 - ll(i,1)^2 );
Minb(i,1) = Rd(i,1);


% if timeD(i,1)<=0% || f6(i,1)>=90 || f6(i,1)<=-90%|| abs(Maxa(i,1)-Maxa(i-1,1))>.2 || Maxa(i,1)<0.15
%     Maxa(i) = 0;
% % % end
% % % if timeD(i,1)<0 || abs(Maxb(i,1)-Maxb(i-1,1))>.2 || Maxb(i,1)<0.15
%     Maxb(i) = 0;
% % % end
% % % if timeD(i,1)<0 || abs(Mina(i,1)-Mina(i-1,1))>.2 || Mina(i,1)<0.15
%     Mina(i) = 0;
% % % end
% % % if timeD(i,1)<0 || abs(Minb(i,1)-Minb(i-1,1))>.2 || Minb(i,1)<0.15
%     Minb(i) = 0;
% end
...
% % plot( timeD(:,1), Mina(:,1), 'c--', 'linewidth', 4 );
% plot( timeD(:,1), D_L1R2(:,1), 'r-', 'linewidth', 4 );
% plot( timeD(:,1), D_L2R1(:,1), 'b-', 'linewidth', 4 );
% legend( 'Max', 'L1R2', 'L2R1' );
% xlabel( 'Time [s]' );
% ylabel( 'Distance [m]' );
% % xlim([ timeD(1,1), 3 ]);
% xlim([ timeD(1,1), timeD(end,1) ]);
% ylim([ 0.18, 0.38 ]);
% fig01 = [ figpath, '/', timepath, '_001' ];
% png01 = [ pngpath, '/', timepath, '_001' ];
% saveas( figure(800), [ fig01, '.fig' ] );
% saveas( figure(800), [ png01, '.png' ] );
% n_figure = n_figure + 1;
...
% xlim([ timeD(1,1), timeD(end,1) ]);
% % ylim([ 0.15, 0.42 ]);
% fig01 = [ figpath, '/', timepath, '_801' ];
% png01 = [ pngpath, '/', timepath, '_801' ];
% saveas( figure(801), [ fig01, '.fig' ] );
% saveas( figure(801), [ png01, '.png' ] );
% n_figure = n_figure + 1;
...
xlim([ mat_13(2,1), mat_13(end,1) ]);
ylim([ -0.04, 0.04 ]);
fig01 = [ figpath, '/', timepath, '_091' ];
png01 = [ pngpath, '/', timepath, '_091' ];
saveas( figure(n_figure), [ fig01, '.fig' ] );
saveas( figure(n_figure), [ png01, '.png' ] );
n_figure = n_figure + 1;
% 
