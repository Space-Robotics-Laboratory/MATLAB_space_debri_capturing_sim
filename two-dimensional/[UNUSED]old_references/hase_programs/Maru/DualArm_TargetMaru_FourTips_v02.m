%%%%%%%%%% SpaceDyn サンプルプログラム  3リンク双腕宇宙ロボットの接触シミュレーション(手先速度制御＋ベースフリー) ターゲット円型
%%%%%%%%%% 結果表示(グラフ描画，動画作成)あり
%%%%%%%%%% DualArm_TargetMaru_Tracking_Contact  2018年8月6日 長谷作成
...
for tm_X_i =    0%-.04;
for tm_Y_i =    0%.02%.1+.04;
for tm_Vx_i =   0
for tm_Vy_i =   0%.05;
for tm_Deg_i =  0
for tm_W_i =    1%0%-.5%1 %1:2:3;
for Theta_h_i = 0
for Kh_i = 0
for Vh_ini_i = 0
for tm_Error_Length_i = 1/4;       % 円幾何中心から，半径の何割の距離に重心があるか0～1
for tm_Error_Theta_i = 45%:45:360;

zeta = [ d_time, 10, Theta_h_i, tm_X_i, tm_Y_i, tm_W_i, Vh_ini_i, Kh_i, tm_Deg_i, tm_Error_Length_i, tm_Error_Theta_i, tm_Vx_i, tm_Vy_i ]';
...
endtime = zeta(2,1);
    ...
    tmX_name = [ 'Xtm=', num2str( tm_X_i ) ];
    tmY_name = [ 'Ytm=', num2str( tm_Y_i ) ];
    tmVx_name = [ 'Vxtm=', num2str( tm_Vx_i ) ];
    tmVy_name = [ 'Vytm=', num2str( tm_Vy_i ) ];
    tmDeg_name = [ 'THtm=', num2str( tm_Deg_i ) ];
    tmW_name = [ 'Wtm=', num2str( tm_W_i ) ];
    tmErLe_name = [ 'ErL=', num2str( tm_Error_Length_i ) ];
    tmErTH_name = [ 'ErTH=', num2str( tm_Error_Theta_i ) ];
%     Theta_h_name = [ 'THh=', num2str( Theta_h_i ) ];
%     Kh_name = [ 'Kh=', num2str( Kh_i ) ];
%     Vh_ini_name = [ 'Vh=', num2str( Vh_ini_i ) ];

zetaname = [ d_Time_name, conma, EndTime_name, conma, tmX_name, conma, tmY_name, conma, ...
             tmVx_name, conma, tmVy_name, conma, tmDeg_name, conma, tmW_name, conma, ...
             tmErLe_name, conma, tmErTH_name ];
...
% ターゲットリンクパラメータ
LP_tm = TargetMaru_FourTips_LP();
SV_tm = TargetMaru_FourTips_SV( LP_tm );
...
num_eL = 1;
jointsL = j_num( LP_d, num_eL );   %左手
num_eR = 2;
jointsR = j_num( LP_d, num_eR );   %右手

% ベースの初期位置・姿勢・速度・角速度
SV_d.R0 = [ 0 0 0 ]';
SV_d.Q0 = [ 0 0 deg2rad( 0 ) ]';
...
SV_tm = f_dyn_rk2( LP_tm, SV_tm );
% 各リンクの座標変換行列(方向余弦行列)の計算（リンクi → 慣性座標系）
SV_d = calc_aa( LP_d, SV_d );
% 各リンク重心の位置
SV_d = calc_pos( LP_d, SV_d );
% 全関節位置姿勢の計算
[ POS_j_L, ORI_j_L ] = f_kin_j( LP_d, SV_d, jointsL );   % 左手
...
theta = pi/5;
L = 0.07;
% L = abs( ( ( r_tip + r_target ) * sin( pi/4 ) ) / ( sin( theta ) ) );        %0.07*1.5;
D = L * cos( theta ) + sqrt( ( r_tip + r_target )^2 - ( L * sin( theta ) )^2 );   % 手先がターゲットに丁度触れるときのターゲット幾何中心からPOS_j4,8の長さを計算
...
%%%%%%%%%% ターゲット初期値設定 %%%%%%%%%%
% ターゲットの姿勢・速度・角速度
SV_tm.v0 = [ zeta(12,1) zeta(13,1) 0 ]';   % ターゲット初期並進速度
SV_tm.w0 = [ 0 0 zeta(6,1) ]';   % ターゲット初期角速度
SV_tm_Q0_0 = [ 0 0 deg2rad( zeta(9,1) ) ]';   % time=0のときの姿勢
SV_tm.Q0 = SV_tm_Q0_0 - [ 0 0 minus_time * SV_tm.w0(3,1) ]';   % time=-minus_timeのときのターゲット初期姿勢
SV_tm.A0 = rpy2dc( SV_tm.Q0 )';

% ターゲットの初期位置   ts_dammy_R0を正方形中心とする．
tm_geo_0 = [ zeta(4,1) ( POS_eL1(2,1) + POS_eL2(2,1) ) / 2 + zeta(5,1) 0 ]';
error_tmR0 = r_target * zeta(10,1);   % 円の中心がSV_tm.R0から error_R0[m] の位置にある
theta_tmR0 = deg2rad( zeta(11,1) );   % 円の中心がSV_tm.R0から theta_R0[deg] の位置にある
SV_tm_R0_0(1,1) = tm_geo_0(1,1) - error_tmR0 * cos( theta_tmR0 + SV_tm_Q0_0(3,1) );   % time=0のときの重心位置
SV_tm_R0_0(2,1) = tm_geo_0(2,1) - error_tmR0 * sin( theta_tmR0 + SV_tm_Q0_0(3,1) );
SV_tm_R0_0(3,1) = tm_geo_0(3,1);
SV_tm.R0 = SV_tm_R0_0 - minus_time * SV_tm.v0;   % time=-minus_timeのときの初期重心位置
tm_geo(1,1) = SV_tm.R0(1,1) + error_tmR0 * cos( theta_tmR0 + SV_tm.Q0(3,1) );
tm_geo(2,1) = SV_tm.R0(2,1) + error_tmR0 * sin( theta_tmR0 + SV_tm.Q0(3,1) );
tm_geo(3,1) = SV_tm.R0(3,1);

% ターゲットの初期観測点位置
r_Obs1 = r_target * 5/5; r_Obs2 = r_target * 5/5;
LeftUp = 3*pi/4; LeftDown = -3*pi/4; RightUp = pi/4; RightDown = -pi/4;
kansoku1 = LeftUp; kansoku2 = LeftDown;

Obs1 = tm_geo + r_Obs1 * [ cos( kansoku1 + SV_tm.Q0(3,1) ) sin( kansoku1 + SV_tm.Q0(3,1) ) 0 ]';   % 初期shikaku1と同じ点
Obs2 = tm_geo + r_Obs2 * [ cos( kansoku2 + SV_tm.Q0(3,1) ) sin( kansoku2 + SV_tm.Q0(3,1) ) 0 ]';   % 初期shikaku2と同じ点
...
est_r_target = sqrt( ( Obs1(1,1) - Obs2(1,1) )^2 + ( Obs1(2,1) - Obs2(2,1) )^2 ) / 2;
...
PointC_tmp_L1 = zeros(3,1); PointC_tmp_L2 = zeros(3,1); PointC_tmp_R1 = zeros(3,1); PointC_tmp_R2 = zeros(3,1);
...
% l_side_min = r_tip + r_target;   % 接触する瞬間のロボットアーム手先球中心～ターゲット中心までの距離 = 手先半径 + ターゲットの一辺/2
% l_corner_min = r_tip + r_target * sqrt(2);   % 接触する瞬間のロボットアーム手先球中心～ターゲット中心までの距離 = 手先半径 + ターゲットの対角/2
l_min = r_tip + r_target;
...
% ターゲット重心 運動
fo_05_MTargetCoM_Motion = fopen( [ datpath, '/', timepath, '_', '05_MTargetCoM_Motion.dat' ], 'w' );   % ターゲットの運動に関する変数　位置，速度，加速度，姿勢，角速度，各加速度
% ターゲット手先 運動
fo_06_MTargetEE_Motion = fopen( [ datpath, '/', timepath, '_', '06_MTargetEE_Motion.dat' ], 'w' );   % ターゲット手先の運動に関する変数　四角頂点位置，接触位置(ターゲット重心座標系)，接触位置速度
% ロボット 力学
...
% ターゲット 力学
fo_08_MTarget_Kinetics = fopen( [ datpath, '/', timepath, '_', '08_MTarget_Kinetics.dat' ], 'w' );   % ターゲットの力学に関する変数　(ターゲットが受ける) 外力，外トルク
% 接触フラグ
...
% ターゲット アニメーション用
fo_16_MTarget_ForMovie = fopen( [ datpath, '/', timepath, '_', '16_MTarget_ForMovie.dat' ], 'w' );   % 重心位置，四角頂点位置，重心姿勢
% ターゲット アニメーション用
...
fprintf( fo_01_Length_etc, [ repmat( s, 1, 16 ), se ], ...   % =17コ
 'r_tip', 'd_target', 'r_target', 'r_motor', 'r_base', 'r_airtank', ...   % 6
...
fprintf( fo_05_MTargetCoM_Motion, [ repmat( s, 1, 18 ), se ], ...   % =19
 'time [1]', 'SV_tm.R0 [3]',u,u, 'SV_tm.v0 [3]',u,u, 'SV_tm.vd0 [3]',u,u, 'SV_tm.Q0 [3]',u,u, 'SV_tm.w0 [3]',u,u, 'SV_tm.wd0 [3]',u,u );   % 19
% ターゲット手先 運動   % ターゲット手先の運動に関する変数　四角頂点位置，接触位置(ターゲット重心座標系)，接触位置速度
fprintf( fo_06_MTargetEE_Motion, [ repmat( s, 1, 36 ), se ], ...   % =37
...
fprintf( fo_08_MTarget_Kinetics, [ repmat( s, 1, 6 ), se ], ...   % =7
 'time [1]', 'SV_tm.F0 [3]',u,u, 'SV_tm.T0 [3]',u,u );   % 7
% 接触フラグ   % 接触判定(1~4)
...
fprintf( fo_11_Estimation, [ repmat( s, 1, 31 ), se ], ....   % 32
 'time [1]', 'Obs1 [3]',u,u, 'Obs2 [3]',u,u, 'est_P [3]',u,u, 'est_V [3]',u,u, 'est_Q [3]',u,u, 'est_q [1]', 'est_W [3]',u,u, ...   % 20
             'est_geo_center [3]',u,u, 'tm_geo [3]',u,u, 'est_r1 [1]', 'est_r2 [1]', 'est_phi1 [1]', 'est_phi2 [1]', 'est_R12 [1]', 'est_Theta12 [1]' );   % 12
...
fprintf( fo_13_Momentum_ForMovie, [ repmat( s, 1, 36 ), se ], ...   % =37
 'time [1]', 'PP_d [6]',u,u,u,u,u, 'PP_tm [6]',u,u,u,u,u, 'PP [6]',u,u,u,u,u, 'PP_d_2 [6]',u,u,u,u,u, 'PP_tm_2 [6]',u,u,u,u,u, 'PP_2 [6]',u,u,u,u,u );   % 37
...
fprintf( fo_16_MTarget_ForMovie, [ repmat( s, 1, 18 ), se ], ...   % =19
 'time [1]', 'SV_tm.R0 [3]',u,u, 'shikaku1 [3]',u,u, 'shikaku2 [3]',u,u, 'shikaku3 [3]',u,u, 'shikaku4 [3]',u,u, 'SV_tmf.Q0 [3]',u,u );   % 19

% dPOS
fprintf( fo_17_memo, [ repmat( s, 1, 37 ), se ], ...   % =37
 'time [1]', 'd_POSL [3]',u,u, 'dPOSR [3]',u,u,'xeL_des(1:2)',u, 'xeR_des(1:2)',u, ...
             'd_ten1',u,u, 'd_ten2',u,u, 'd_ten3',u,u, 'd_ten4',u,u, 'QL3', 'QR7', 'QL2', 'QR6', 'd_QL3', 'd_QR7', 'Phase', 'Catchtime', 'flagphase5', 'fff', 'd_geo',u,u, 'delta_t', 'delta_q' );   % 38
...
if      pi/2 <= SV_tm.Q0(3,1) && SV_tm.Q0(3,1) < pi    % ターゲットがπ/2～πに傾くとき
    q0 = SV_tm.Q0(3,1) - pi/2;
elseif -pi/2 <= SV_tm.Q0(3,1) && SV_tm.Q0(3,1) < 0     % ターゲットが-π/2～0に傾くとき
    q0 = SV_tm.Q0(3,1) + pi/2;
elseif -pi <= SV_tm.Q0(3,1) && SV_tm.Q0(3,1) < -pi/2   % ターゲットが-π～-π/2に傾くとき
    q0 = SV_tm.Q0(3,1) + pi;
...
= Shikaku_FourTips_CONTACT_FLAG( SV_tm, q0, d_target, r_tip, tm_geo, POS_eL1, POS_eL2, POS_eR1, POS_eR2 );
%}

[ l_L1, l_L2, l_R1, l_R2, l_surf_L1, l_surf_L2, l_surf_R1, l_surf_R2, contactflag_L1, contactflag_L2, contactflag_R1, contactflag_R2 ] ...
...
%%%%%%%%%%%%%%%%%%%% 接触力計算 %%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
[ contactflag_L1, F_NN_L1, FR_N_L1, FR_T_L1, norm_L1, tang_L1, FR_L1, ...
  delta_L1, deltaVel_L1, delta_tmp_L1, TB0_L1, DB0_L1, curPosAP33_L1, curPosAP3_delta_L1, curPosAP3_tmp_L1, curPosBP33_L1, PointC_L1, PointC_tmp_L1 ] ...
= Maru_FourTips_LEFT_CONTACT_v01( SV_d, SV_tm, d_time, r_target, r_tip, contactflag_L1, l_min, l_surf_L1, tm_geo, Rg_d, POS_eL1, delta_tmp_L1, curPosAP3_tmp_L1, PointC_tmp_L1, cof, kkk, ccc );
[ contactflag_L2, F_NN_L2, FR_N_L2, FR_T_L2, norm_L2, tang_L2, FR_L2, ...
  delta_L2, deltaVel_L2, delta_tmp_L2, TB0_L2, DB0_L2, curPosAP33_L2, curPosAP3_delta_L2, curPosAP3_tmp_L2, curPosBP33_L2, PointC_L2, PointC_tmp_L2 ] ...
= Maru_FourTips_LEFT_CONTACT_v01( SV_d, SV_tm, d_time, r_target, r_tip, contactflag_L2, l_min, l_surf_L2, tm_geo, Rg_d, POS_eL2, delta_tmp_L2, curPosAP3_tmp_L2, PointC_tmp_L2, cof, kkk, ccc );
[ contactflag_R1, F_NN_R1, FR_N_R1, FR_T_R1, norm_R1, tang_R1, FR_R1, ...
  delta_R1, deltaVel_R1, delta_tmp_R1, TB0_R1, DB0_R1, curPosAP33_R1, curPosAP3_delta_R1, curPosAP3_tmp_R1, curPosBP33_R1, PointC_R1, PointC_tmp_R1 ] ...
= Maru_FourTips_RIGHT_CONTACT_v01( SV_d, SV_tm, d_time, r_target, r_tip, contactflag_R1, l_min, l_surf_R1, tm_geo, Rg_d, POS_eR1, delta_tmp_R1, curPosAP3_tmp_R1, PointC_tmp_R1, cof, kkk, ccc );
[ contactflag_R2, F_NN_R2, FR_N_R2, FR_T_R2, norm_R2, tang_R2, FR_R2, ...
  delta_R2, deltaVel_R2, delta_tmp_R2, TB0_R2, DB0_R2, curPosAP33_R2, curPosAP3_delta_R2, curPosAP3_tmp_R2, curPosBP33_R2, PointC_R2, PointC_tmp_R2 ] ...
= Maru_FourTips_RIGHT_CONTACT_v01( SV_d, SV_tm, d_time, r_target, r_tip, contactflag_R2, l_min, l_surf_R2, tm_geo, Rg_d, POS_eR2, delta_tmp_R2, curPosAP3_tmp_R2, PointC_tmp_R2, cof, kkk, ccc );
...
= Shikaku_FourTips_LEFT_CONTACT_v03( SV_d, SV_tm, d_time, r_target, r_tip, contact_flag_L1, contactflag_L1, shikaku1, shikaku2, shikaku3, shikaku4, ...
                                      l_side_min, l_corner_min, tm_geo, q0, Rg_d, POS_eL1, delta_tmp_L1, curPosAP3_tmp_L1, l1, l2, l3, l4, g1, g2, g3, g4, cof, kkk, ccc );
...
= Shikaku_FourTips_LEFT_CONTACT_v03( SV_d, SV_tm, d_time, r_target, r_tip, contact_flag_L2, contactflag_L2, shikaku1, shikaku2, shikaku3, shikaku4, ...
                                      l_side_min, l_corner_min, tm_geo, q0, Rg_d, POS_eL2, delta_tmp_L2, curPosAP3_tmp_L2, l1, l2, l3, l4, g1, g2, g3, g4, cof, kkk, ccc );
%%%%%%%%%% 右側3,4 %%%%%%%%%%
...
= Shikaku_FourTips_RIGHT_CONTACT_v03( SV_d, SV_tm, d_time, r_target, r_tip, contact_flag_R1, contactflag_R1, shikaku1, shikaku2, shikaku3, shikaku4, ...
                                      l_side_min, l_corner_min, tm_geo, q0, Rg_d, POS_eR1, delta_tmp_R1, curPosAP3_tmp_R1, l1, l2, l3, l4, g1, g2, g3, g4, cof, kkk, ccc );
...
= Shikaku_FourTips_RIGHT_CONTACT_v03( SV_d, SV_tm, d_time, r_target, r_tip, contact_flag_R2, contactflag_R2, shikaku1, shikaku2, shikaku3, shikaku4, ...
                                      l_side_min, l_corner_min, tm_geo, q0, Rg_d, POS_eR2, delta_tmp_R2, curPosAP3_tmp_R2, l1, l2, l3, l4, g1, g2, g3, g4, cof, kkk, ccc );
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
SV_tm.F0 = -( FR_L1 + FR_L2 + FR_R1 + FR_R2 );      % ターゲットが受ける力(左右からの合力) F3_1(左手,慣性右方向) + F3_2(右手,慣性左方向)
SV_tm.T0 = T_t3_L1 + T_t3_L2 + T_t3_R1 + T_t3_R2;    % ターゲットが受けるトルク(左右からの合トルク)
FR_L = FR_L1 + FR_L2;
FR_R = FR_R1 + FR_R2;
...
est_Q_tmp = est_Q;
% 更新   % ここは未知情報を代入してよい
Obs1 = tm_geo + r_Obs1 * [ cos( kansoku1 + SV_tm.Q0(3,1) ), sin( kansoku1 + SV_tm.Q0(3,1) ), 0 ]' + [ randi(10,1) * delta, randi(10,1) * delta, 0 ]';   % 初期shikaku1と同じ点を観測
Obs2 = tm_geo + r_Obs2 * [ cos( kansoku2 + SV_tm.Q0(3,1) ), sin( kansoku2 + SV_tm.Q0(3,1) ), 0 ]' + [ randi(10,1) * delta, randi(10,1) * delta, 0 ]';   % 初期shikaku2と同じ点を観測

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%% 重心位置の推定 %%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
[ est_P, est_V, est_erP, NorTraj_12, est_Q, est_q, est_W, est_geo, est_r_target, est_r1, est_r2, est_phi1, est_phi2, est_R12, est_Theta12 ] ...
...
if     Phase == 4 && flagphase5 ~= 1    % time >= round( (t0+delta_t)/d_time )*d_time   % flag_kotei == 1
    SV_d.tau = H_asuta * qdd + C_asuta - JL' * Fh_L - JR' * Fh_R ...   % 位置制御 
...
% ターゲット姿勢計算
SV_tm.Q0 = dc2rpy( SV_tm.A0' );   % ターゲットの方向余弦行列を角度に変換
Q0_deg_t = rad2deg( SV_tm.Q0 );
...
% ターゲット
  HH_tm = calc_hh( LP_tm, SV_tm );
  Hb_tm = HH_tm( 1:6, 1:6 );
  PP_tm = Hb_tm * [ SV_tm.v0' SV_tm.w0' ]';
...
%   R_ts = SV_ts.R0 - Rg_d;
  ...
  PP( 4:5, 1 ) = zeros( 2, 1 );
  PP( 6, 1 ) = PP_d( 6, 1 ) + PP_tm( 6, 1 ) + LP_tm.mass * ( R( 1, 1 ) * SV_tm.v0( 2, 1 ) - R( 2, 1 ) * SV_tm.v0( 1, 1 ) );
...
%   PP_d_2 = [ P_d' L_d' ]';
% % ターゲット重心位置の歪対称行列
%   target_tilde = tilde( SV_tm.R0 );
% % ターゲットの並進運動量P_t, 角運動量L_t
%   P_tm = LP_tm.m0 * SV_tm.v0;
%   L_tm = LP_tm.inertia0 * SV_tm.w0 + target_tilde * P_tm;  
%   PP_tm_2 = [ P_tm' L_tm' ]';
% % 系全体の運動量
%   PP_2 = PP_d_2 + PP_tm_2;
...
% ターゲット重心 運動   % ターゲットの運動に関する変数　位置，速度，加速度，姿勢，角速度，各加速度
fprintf( fo_05_MTargetCoM_Motion, [ repmat( g, 1, 18 ), ge ], ...   % =19
         display_time, SV_tm.R0, SV_tm.v0, SV_tm.vd0, SV_tm.Q0, SV_tm.w0, SV_tm.wd0 );   % 19
% ターゲット手先 運動   % ターゲット手先の運動に関する変数　四角頂点位置，接触位置(ターゲット重心座標系)，接触位置速度
fprintf( fo_06_MTargetEE_Motion, [ repmat( g, 1, 36 ), ge ], ...   % =37
...
fprintf( fo_07_Robot_Kinetics, [ repmat( g, 1, 56 ), ge ], ...   % =57
         ...
         SV_d.tau );   % 8
% ターゲット 力学      % ターゲットの力学に関する変数　(ターゲットが受ける) 外力，外トルク
fprintf( fo_08_MTarget_Kinetics, [ repmat( g, 1, 6 ), ge ], ...   % =7
         display_time, SV_tm.F0, SV_tm.T0 );   % 7
% 接触フラグ   % 接触判定(1~4)
...
fprintf( fo_11_Estimation, [ repmat( g, 1, 31 ), ge ], ....   % 32
         ...
         est_geo, tm_geo, est_r1, est_r2, est_phi1, est_phi2, est_R12, est_Theta12 );   % 12
% めり込み   % l_x_H(1~4), delta_H(1~4), deltaVel_H(1~4)
...
fprintf( fo_13_Momentum_ForMovie, [ repmat( g, 1, 36 ), ge ], ...   % =37
         display_time, PP_d, PP_tm, PP, ...   % 19
         PP_d_2, PP_tm_2, PP_2 );   % 18
% 力 アニメーション用   % 法線単位ベクトル，接線単位ベクトル，法線力，接線力
...
fprintf( fo_15_Robot_ForMovie, [ repmat( g, 1, 46 ), ge ], ...   % =47
         ...
         SV_d.Q0 );   % 3
% ターゲット アニメーション用    % 重心位置，関節位置，手先位置，重心姿勢
fprintf( fo_16_MTarget_ForMovie, [ repmat( g, 1, 18 ), ge ], ...   % =19
         display_time, SV_tm.R0, shikaku1, shikaku2, shikaku3, shikaku4, SV_tm.Q0 );   % 19

% d_POS
fprintf( fo_17_memo, [ repmat( g, 1, 37 ), ge ], ...   % =37
         display_time, d_POS_j4, d_POS_j8, xe_des(1:2), xe_des(7:8), d_ten1, d_ten2, d_ten3, d_ten4, QL_3(3,1), QR_7(3,1), QL_2(3,1), QR_6(3,1), d_QL_3, d_QR_7, Phase, catchtime, flagphase5, fff, d_geo, delta_t, delta_q );   % 38
