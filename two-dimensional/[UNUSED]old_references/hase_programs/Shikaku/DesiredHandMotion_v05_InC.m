...
%%% Phase 2
  ...
  %kakuR = kakudo( [ POS_j7' 0 ]', [ POS_j6_st' 0 ]', SV_ts.R0 );
...
%}
 ...
 if ((time - t1) > 1.2) %&& contactflag_L2==1 )
     Phase = 5;
 else
     Phase = 4;
 end
...
%SV_d.w0(3,1) = SV_d.w0(3,1) - (((kakuR - d_q_joints(7,1))) / delta_t) - (((kakuL - d_q_joints(3,1))) / delta_t);
 ...
 %{
 %%% Phase 7 %手先制御Phaseを追加する場合コメントアウトを消してください
    case 7
...
%{
  ...
  vfL = zeros(2,1);   % 目標速度
    a0L = x0L;   % 係数に代入
    a1L = v0L;
    a2L = ( -3 * ( x0L - xfL ) - delta_t_3 * ( 2 * v0L + vfL ) )/( delta_t_3 )^2;
    a3L = ( 2 * ( x0L - xfL ) - delta_t_3 * ( v0L + vfL ) )/( delta_t_3 )^3;
  aL = [ a0L, a1L, a2L, a3L ];
  ...
  vfR = zeros(2,1);   % 目標速度
    a0R = x0R;   % 係数に代入
    a1R = v0R;
    a2R = ( -3 * ( x0R - xfR ) - delta_t_3 * ( 2 * v0R + vfR ) )/( delta_t_3 )^2;
    a3R = ( 2 * ( x0R - xfR ) - delta_t_3 * ( v0R + vfR ) )/( delta_t_3 )^3;
  aR = [ a0R, a1R, a2R, a3R ];

  x_L = a0L + a1L * ( time - ( minus_time_3 ) ) + a2L * ( time - ( minus_time_3 ) )^2 + a3L * ( time - ( minus_time_3 ) )^3;   % スプライン曲線軌跡上の目標手先位置を計算
  x_R = a0R + a1R * ( time - ( minus_time_3 ) ) + a2R * ( time - ( minus_time_3 ) )^2 + a3R * ( time - ( minus_time_3 ) )^3;
%}
% % 角に垂直に
%   kL = kakudo( est_P, [ x_L' 0 ]', tenL );
%   kR = kakudo( est_P, [ x_R' 0 ]', tenR );
...
%SV_d.w0(3,1) = SV_d.w0(3,1) - (((kakuR - d_q_joints(7,1))) / delta_t) + (((kakuL - d_q_joints(3,1))) / delta_t);
 ...
 %xeL_des = [ x_L' zeros(1,3) 0 ]';
 %xeR_des = [ x_R' zeros(1,3) 0 ]';
 xeL_des = [ POS_j4' zeros(1,3) d_q_joints(3,1) ]';
 xeR_des = [ POS_j8' zeros(1,3) d_q_joints(7,1) ]';
 veL_des = zeros(6,1);
 veR_des = zeros(6,1);
 aeL_des = zeros(6,1);
 aeR_des = zeros(6,1);
...
%%% Phase 8
 ...
 % if     est_W(3,1) > 0   % ターゲット角速度が正のとき
  ...
  %x_L = aL(:,1) + aL(:,2) * ( time - ( minus_time_3 ) ) + aL(:,3) * ( time - ( minus_time_3 ) )^2 + aL(:,4) * ( time - minus_time_3 )^3;
  %x_R = aR(:,1) + aR(:,2) * ( time - ( minus_time_3 ) ) + aR(:,3) * ( time - ( minus_time_3 ) )^2 + aR(:,4) * ( time - minus_time_3 )^3;
...
%SV_d.w0(3,1) = SV_d.w0(3,1) - (((kakuR - d_q_joints(7,1))) / delta_t) - (((kakuL - d_q_joints(3,1))) / delta_t);
 ...
 if ((time - minus_time_3) > delta_t_3)
     Phase = 9;
 else
     Phase = 8;
 end
