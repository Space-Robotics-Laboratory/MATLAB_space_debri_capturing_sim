...
%%% Phase 4
elseif (time >= ( t0 + delta_t + minus_time ) && time < ( t0 + minus_time + delta_t + d_time*200 ) || contactflag_L1==1 || contactflag_L2==1 || contactflag_R1==0 || contactflag_R2==0  ||  Phase==4  ||  Phase==5)
Phase = 4;
...
%     end
  ...
  x_R = aR(:,1) + aR(:,2) * ( time - ( t0 + delta_t + minus_time ) ) + aR(:,3) * ( time - ( t0 + delta_t + minus_time ) )^2 + aR(:,4) * ( time - t0 - delta_t - minus_time )^3;
...
if  ten1(1,1) < (tenL(2,1)-bbL)/aaL  &&  (tenR(2,1)-bbR)/aaR < tenR(1,1)  ||  Phase == 5
