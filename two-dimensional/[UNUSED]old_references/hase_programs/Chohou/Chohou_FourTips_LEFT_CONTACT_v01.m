...
= Chohou_FourTips_LEFT_CONTACT_v01( SV_d, SV_tc, d_time, r_tip, contact_flag_L, contactflag_L, shikaku1, shikaku2, shikaku3, shikaku4, ...
                                    l_side_tan_min, l_side_cho_min, l_corner_min, tc_geo, q0, Rg_d, POS_eL, delta_tmp_L, curPosAP3_tmp_L, ...
                                    l1, l2, l3, l4, g1, g2, g3, g4, cof_L, kkk, ccc, half_side_tan, half_side_cho )
...
if     contact_flag_L(1,1) > 0  ||  contact_flag_L(2,2) > 0  ||  contact_flag_L(3,3) > 0  ||  contact_flag_L(4,4) > 0
   ...
   r_L = norm( shikaku_L - POS_eL );   % ターゲット中心から手先球中心までの距離
   l_x_L =  0.0;
% めり込み量設定
...
% 接触面角度
    if shikaku_L(1,1) == POS_eL(1,1)   % 手先球中心とターゲット中心が同じx座標のとき    
      if shikaku_L(2,1) > POS_eL(2,1)   % 手先球の真上にターゲットがある場合
          ...
          norm_L = [ cos(-theta_L_yaw) sin(-theta_L_yaw) 0 ]';   %法線単位ベクトル(ターゲット外側向き，手先が受ける法線力の向き) [0 -1 0]
      else                                 % 手先球の真下にターゲットがある場合
          theta_L_yaw = -pi/2;   % 左に左手先，右にターゲットがある状態を角度0として，接触面は-π/2
          norm_L = [ cos(-theta_L_yaw) sin(-theta_L_yaw) 0 ]';   % [0 1 0]
      end
    else   % 手先球中心とターゲット中心が異なるx座標とき
          theta_L_yaw = atan2( ( shikaku_L(2,1) - POS_eL(2,1) ), ( shikaku_L(1,1) - POS_eL(1,1) ) );   % 逆正接→角度導出  θ1'
...
% ターゲット重心座標系から見た接触位置
    curPosAP3_L = curPosAP3_L - SV_tc.R0;   % APを使って慣性座標中心をターゲット重心座標中心に替える
    TB0_L = rpy2dc( SV_tc.Q0 );   % ターゲット回転角の方向余弦行列を作る
...
% 接触力の計算
  if delta_L >= 0.0  ||  r_tip - r_L >= 0.0  ||  l_corner_min >= ( sqrt( ( tc_geo(1,1) - POS_eL(1,1) )^2 + ( tc_geo(2,1) - POS_eL(2,1) )^2 ) - r_tip )  % めり込み量が正のとき(接触した瞬間からめり込んでいるとき)
      contactflag_L = 1;
...
% 摩擦力の計算
  if delta_L >= 0.0  ||  r_tip - r_L >= 0.0  ||  l_corner_min >= ( sqrt( ( tc_geo(1,1) - POS_eL(1,1) )^2 + ( tc_geo(2,1) - POS_eL(2,1) )^2 ) - r_tip )

    if     contact_flag_L(1,1) > 0
        psi_L = abs( pi/2 - abs( q0 + g_L - theta_L_yaw ) );
        corner_vel_L = l_tcR0 * SV_tc.w0(3,1);   % 四角の頂点の速度(ターゲット重心から頂点までの距離×ターゲットの角速度)
    elseif contact_flag_L(2,2) > 0
        psi_L = abs( q0 + g_L - theta_L_yaw );
        corner_vel_L = l_tcR0 * SV_tc.w0(3,1);   % 四角の頂点の速度(ターゲット重心から頂点までの距離×ターゲットの角速度)
    elseif contact_flag_L(3,3) > 0
        psi_L = abs( pi/2 - abs( q0 + g_L - theta_L_yaw ) );
        corner_vel_L = l_tcR0 * SV_tc.w0(3,1);   % 四角の頂点の速度(ターゲット重心から頂点までの距離×ターゲットの角速度)
   ...
   elseif POS_eL(1,1) > SV_tc.R0(1,1)
...
if     ( contact_flag_L(1,2) > 0 && ( 0 <= q0 && q0 < pi/2 ) ) || ( contact_flag_L(4,1) > 0 && ( pi/2 <= q0 && q0 < pi ) )
   if contact_flag_L(4,1) > 0
      q0 = q0 - pi/2;
   ...
   l_L = norm( tc_geo - POS_eL );   % ターゲット中心から左手先球中心までの距離 正
    if     0 <= q0  &&  q0 < pi/4   % この分岐内容が接触領域によって変わります
        if     POS_eL(2,1) > tc_geo(2,1)
            phi_L = abs( acos( abs( POS_eL(1,1) - tc_geo(1,1) ) / l_L ) + q0 );   % ターゲット中心位置と手先中心位置を結ぶ斜辺と，ターゲット中心を通りターゲット四角形の上下辺に平行な直線が作る底辺を用いた，直角三角形の鋭角φ
        elseif POS_eL(2,1) < tc_geo(2,1)
            phi_L = abs( acos( abs( POS_eL(1,1) - tc_geo(1,1) ) / l_L ) - q0 );
        else
            phi_L = q0;
        end
    elseif pi/4 < q0  &&  q0 < pi/2
        if     POS_eL(1,1) < tc_geo(1,1)
            phi_L = abs( acos( abs( POS_eL(1,1) - tc_geo(1,1) ) / l_L ) - q0 );
        elseif POS_eL(1,1) > tc_geo(1,1)
            phi_L = abs( pi - acos( abs( POS_eL(1,1) - tc_geo(1,1) ) / l_L ) - q0 );
        else
            phi_L = abs( pi/2 - q0 );
        end
    else   % q0 = π/4のとき
            phi_L = abs( acos( abs( POS_eL(1,1) - tc_geo(1,1) ) / l_L ) - pi/4 );
    end
...
elseif ( contact_flag_L(2,3) > 0 && ( 0 <= q0 && q0 < pi/2 ) ) || ( contact_flag_L(1,2) > 0 && ( pi/2 <= q0 && q0 < pi ) )
   ...
   l_L = norm( tc_geo - POS_eL );   % ターゲット中心から手先球中心までの距離 正
    if     0 <= q0  &&  q0 < pi/4   % この分岐内容が接触領域によって変わります
        if     POS_eL(1,1) < tc_geo(1,1)
            phi_L = abs( acos( abs( POS_eL(2,1) - tc_geo(2,1) ) / l_L ) + q0 );   % ターゲット中心位置と手先中心位置を結ぶ斜辺と，ターゲット中心を通りターゲット四角形の上下辺に平行な直線が作る底辺を用いた，直角三角形の鋭角φ
        elseif POS_eL(1,1) > tc_geo(1,1)
            phi_L = abs( acos( abs( POS_eL(2,1) - tc_geo(2,1) ) / l_L ) - q0 );
        else
            phi_L = q0;
        end
    elseif pi/4 < q0  &&  q0 < pi/2
        if     POS_eL(2,1) < tc_geo(2,1)
            phi_L = abs( acos( abs( POS_eL(2,1) - tc_geo(2,1) ) / l_L ) - q0 );
        elseif POS_eL(2,1) > tc_geo(2,1)
            phi_L = abs( pi - acos( abs( POS_eL(2,1) - tc_geo(2,1) ) / l_L ) - q0 );
        else
            phi_L = abs( pi/2 - q0 );
        end
    else   % q0 = π/4のとき
            phi_L = abs( acos( abs( POS_eL(2,1) - tc_geo(2,1) ) / l_L ) - pi/4 );
    end
   l_x_L = l_L * cos( phi_L );   % ターゲット重心座標における左手先球中心のx座標の絶対値
...
elseif ( contact_flag_L(3,4) > 0 && ( 0 <= q0 && q0 < pi/2 ) ) || ( contact_flag_L(2,3) > 0 && ( pi/2 <= q0 && q0 < pi ) )
   if contact_flag_L(2,3) > 0
      q0 = q0 - pi/2;
      l_side_min = l_side_cho_min;
      half_side = half_side_cho;
   else
...
elseif ( contact_flag_L(4,1) > 0 && ( 0 <= q0 && q0 < pi/2 ) ) || ( contact_flag_L(3,4) > 0 && ( pi/2 <= q0 && q0 < pi ) )
   if contact_flag_L(3,4) > 0
      q0 = q0 - pi/2;
      l_side_min = l_side_tan_min;
      half_side = half_side_tan;
   else
