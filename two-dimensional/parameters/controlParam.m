...
controlParam.captureTime = .2;                 % 接近後，捕獲に要する時間

%% Used in One Hand Contact
% timing parameters
controlParam.impedanceDuration = .35;            % 接触後，インピーダンス制御を持続させる時間
controlParam.swichingDelay2Direct = .4;        % ターゲット角速度が減衰したのち，直接捕獲モードに切り替える待ち時間
...
controlParam.ki = [1, 1, 1]'*0.05;                   % アドミタンス制御バネ特性   [.5, .5, .5]';

% contact parameters
controlParam.contactPositionRatio = .6;             % 接触位置がターゲット辺のどの割合にあるかを表す.0で中心1で頂点
