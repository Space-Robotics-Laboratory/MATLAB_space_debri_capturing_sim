...
function controlParam = controlParam()
    ...
    controlParam.di = [1, 1, 1]'*2;                   % アドミタンス制御ダンパ特性  [10, 10, 10]';
    controlParam.ki = [1, 1, 1]'*0.5;                   % アドミタンス制御バネ特性   [.5, .5, .5]';
    
    % contact parameters
    controlParam.contactPositionRatio = .95;             % 接触位置がターゲット辺のどの割合にあるかを表す.0で中心1で頂点
