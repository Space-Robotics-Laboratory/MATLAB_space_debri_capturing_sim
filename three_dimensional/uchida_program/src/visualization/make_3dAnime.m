...
function make_3dAnime(datSaver, paths, param)
    % パラメータインポート
    mov_dtime = param.MovDivTime;                              % 動画刻み時間
    dataStruct = datSaver.datStruct;
    
    % force vector scale
    scale = param.general.quiverScale;
    ...
    for count = 1:datSaver.datNum
        ...
        if rem(anime_time, mov_dtime) == 0
            ...
            % ロボ外力描画
            vis_roboForce(endEffecPos, roboForceLeft, roboForceRight, scale, param.general.roboForceColor)

            % ターゲット外力描画
            vis_targetForce(targetR0, targetForce, scale, param.general.targetForceColor)
