...
% Phase = 3;
elseif ( time > ( t0 + minus_time )  && time < ( t0 + delta_t + minus_time )) && max(contactflag_LR) == 0 && Phase ~= 4 && Phase ~= 5 
Phase = 3;
...
%%% Phase 4  %ケージング条件満たすまでの寄せフェーズ
% elseif time >= ( t0 + delta_t + minus_time ) || max(contactflag_LR) > 0  ||  Phase==4  ||  Phase==5 || time >=5
elseif  max(contactflag_LR) > 0 && caging3dim ~=1 && Phase ~= 5 
Phase = 4;

    kakuL = kakudo_3dim( POS_jL5, POS_jL4_st, POS_jR5 ); % 目標手先角度
    kakuR = kakudo_3dim( POS_jR5, POS_jR4_st, POS_jL5 );
    
%     kakuL = kakudo_3dim( POS_jL5, POS_jL4_st, ts6); % 目標手先角度
%     kakuR = kakudo_3dim( POS_jR5, POS_jR4_st, ts3);
%     kakuL = kakudo_3dim( POS_jL5, POS_jL4_st, SV_ts.R0 ); % 目標手先角度
%     kakuR = kakudo_3dim( POS_jR5, POS_jR4_st, SV_ts.R0 );
%     kakuL = kakudo_3dim( POS_jL5, POS_jL4_st, POS_ee4 ); % 目標手先角度
%     kakuR = kakudo_3dim( POS_jR5, POS_jR4_st, POS_ee1 );
%     tenL = ts6;
%     tenR = ts3;
%   kakuL = SV_ts.R0 - POS_jL5;
%   kakuR = SV_ts.R0 - POS_jR5;
...
x_R = ts3m;
 ...
 xeL_des = [ x_L' kakuL ]'; %Phase4は POS_eL,eR の値が0
 xeR_des = [ x_R' kakuR ]';
 veL_des = zeros(6,1);
 veR_des = zeros(6,1);
 aeL_des = zeros(6,1);
