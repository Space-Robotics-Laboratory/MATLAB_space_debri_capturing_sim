%%%%%%%%%% SpaceDyn サンプルプログラム  3リンク双腕宇宙ロボットの接触シミュレーション(手先速度制御＋ベースフリー) ターゲット長方形型
%%%%%%%%%% 結果表示(グラフ描画，動画作成)あり
%%%%%%%%%% DualArm_TargetChohou_Tracking_Contact  2018年8月6日 長谷作成
...
for tc_X_i =    0%0.080%-0.03%
for tc_Y_i =    0;
for tc_Vx_i =   0%-0.1%
for tc_Vy_i =   0;
for tc_Deg_i =  0;%(pi-(pi-1.1071)) * 180/pi%(1.1071) * 180/pi%
for tc_W_i =    1;%-3%0%1%
for Theta_h_i = 0
for Kh_i = 0
for Vh_ini_i = 0
for tc_Error_Length_i = 4/4%1/2       % 長方形幾何中心から，長方形半短辺の何割の距離に重心があるか0～1
for tc_Error_Theta_i = 45

zeta = [ d_time, 10, Theta_h_i, tc_X_i, tc_Y_i, tc_W_i, Vh_ini_i, Kh_i, tc_Deg_i, tc_Error_Length_i, tc_Error_Theta_i, tc_Vx_i, tc_Vy_i ]';
...
endtime = zeta(2,1);
    ...
    conma = ',';
    d_Time_name = [ 'dt=', num2str( zeta(1,1) ) ];
    EndTime_name = [ 'et=', num2str( zeta(2,1) ) ];
    tcX_name = [ 'Xtc=', num2str( tc_X_i ) ];
    tcY_name = [ 'Ytc=', num2str( tc_Y_i ) ];
    tcVx_name = [ 'Vxtc=', num2str( tc_Vx_i ) ];
    tcVy_name = [ 'Vytc=', num2str( tc_Vy_i ) ];
    tcDeg_name = [ 'THtc=', num2str( tc_Deg_i ) ];
    tcW_name = [ 'Wtc=', num2str( tc_W_i ) ];
    tcErLe_name = [ 'ErL=', num2str( tc_Error_Length_i ) ];
    tcErTH_name = [ 'ErTH=', num2str( tc_Error_Theta_i ) ];
%     Theta_h_name = [ 'THh=', num2str( Theta_h_i ) ];
%     Kh_name = [ 'Kh=', num2str( Kh_i ) ];
%     Vh_ini_name = [ 'Vh=', num2str( Vh_ini_i ) ];

zetaname = [ d_Time_name, conma, EndTime_name, conma, tcX_name, conma, tcY_name, conma, ...
             tcVx_name, conma, tcVy_name, conma, tcDeg_name, conma, tcW_name, conma, ...
             tcErLe_name, conma, tcErTH_name ];
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% ロボットリンクパラメータ
LP = DualArm_FourTips_LP_v3();   
SV = DualArm_FourTips_SV_v3( LP );
% ターゲットリンクパラメータ
LP_tc = TargetChohou_FourTips_LP();
SV_tc = TargetChohou_FourTips_SV( LP_tc );
...
figfile = [ path, '/', timepath, '-', 'fig' ];
pngfile = [ path, '/', timepath, '-', 'png' ];
movfile = [ path, '/', timepath, '-', 'mov' ];
mkdir( datfile ); mkdir( figfile ); mkdir( pngfile ); mkdir( movfile );

% パス設定
datpath = [ path, '/', timepath, '-', 'dat' ];
...
theta_contact = [ deg2rad(0)  deg2rad(10)  deg2rad(20)  deg2rad(30)  deg2rad(40) ];
...
c_p = polyfit( theta_contact, cc, 3 );
myu_p = polyfit( theta_contact, myu, 3 );

theta_h = deg2rad( zeta(3,1) );   % 追従角度
...
side_target_tan = 0.16;  % ターゲット短辺 16cm
side_target_cho = side_target_tan * chotan;  % ターゲット長辺 (短辺の2倍)
half_side_tan = side_target_tan / 2;
half_side_cho = side_target_cho / 2;
base_yoko = 0.16;   % ベース四角形横 16cm
base_tate = 0.16;   % ベース四角形縦 16cm
base_jushin_teihen = 0.09754;   % ベース重心から底辺までの長さ 9.754cm
base_arm_yoko_1 = abs( LP.c0(1,1) );
base_arm_yoko_2 = abs( LP.c0(1,4) );
base_arm_tate_1 = abs( LP.c0(2,1) );
base_arm_tate_2 = abs( LP.c0(2,4) );
...
num_eL = 1;
jointsL = j_num( LP, num_eL );   %左手
num_eR = 2;
jointsR = j_num( LP, num_eR );   %右手
...
% % ロボットの初期関節角度を設定
q_L = [ pi/3+qqq -pi/2-qqq -pi/3 0 ]';
q_R = -q_L;   % 右側
SV.q = [ q_L' q_R' ]';
q_joints_kotei = zeros(8,1);
kotei1 = pi/16;
kotei2 = pi/6;
q_joints_kotei(1:3,1) = [ pi/3-kotei1 -pi/2+kotei1-kotei2 -pi/3+kotei2 ]'; %[ pi/3+(-pi/7*1.2) -pi/2-(-pi/7*1.2) -pi/3 ]';
q_joints_kotei(5:7,1) = -q_joints_kotei(1:3,1);

%%%%%%%%%% 初期手先位置・姿勢の計算 %%%%%%%%%%
% 順動力学計算
SV =  f_dyn_rk2( LP, SV );
SV_tc = f_dyn_rk2( LP_tc, SV_tc );
% 各リンクの座標変換行列(方向余弦行列)の計算（リンクi → 慣性座標系）
SV = calc_aa( LP, SV );
% 各リンク重心の位置
SV = calc_pos( LP, SV );
% 全関節位置姿勢の計算
[ POS_j_L, ORI_j_L ] = f_kin_j( LP, SV, jointsL );   % 左手
[ POS_j_R, ORI_j_R ] = f_kin_j( LP, SV, jointsR );   % 右手
% 各関節の位置 x,y
...
% D = abs( L_hiraki * cos( theta_hiraki ) + ( r_tip + half_side_tan ) * cos( pi/4 ) );   % 手先が丁度ターゲットに触れるような長さを計算
D = abs( L_hiraki * ( cos( theta_hiraki ) - sin( theta_hiraki ) ) + r_tip * sqrt(2) );   % 手先が丁度ターゲットに触れるような長さを計算
D_0 = abs( L_hiraki * cos( theta_hiraki ) + r_tip + half_side_tan );
[ POS_eL, ORI_eL ] = f_kin_e( LP, SV, jointsL );   % 左手
Qe_radL1 = dc2rpy( ORI_eL' );   %左側
Qe_radL2 = dc2rpy( ORI_eL' );   %左側
POS_eL1 = [ POS_j4' 0 ]' + rpy2dc([ 0 0  theta_hiraki ])' * ( POS_eL - [ POS_j4' 0 ]' ) * L_hiraki / norm( LP.cc( :, 4, 4 ) );
POS_eL2 = [ POS_j4' 0 ]' + rpy2dc([ 0 0 -theta_hiraki ])' * ( POS_eL - [ POS_j4' 0 ]' ) * L_hiraki / norm( LP.cc( :, 4, 4 ) );
[ POS_eR, ORI_eR ] = f_kin_e( LP, SV, jointsR );   % 右手
Qe_radR1 = dc2rpy( ORI_eR' );   %右側
Qe_radR2 = dc2rpy( ORI_eR' );   %右側
POS_eR1 = [ POS_j8' 0 ]' + rpy2dc([ 0 0  theta_hiraki ])' * ( POS_eR - [ POS_j8' 0 ]' ) * L_hiraki / norm( LP.cc( :, 8, 8 ) );
POS_eR2 = [ POS_j8' 0 ]' + rpy2dc([ 0 0 -theta_hiraki ])' * ( POS_eR - [ POS_j8' 0 ]' ) * L_hiraki / norm( LP.cc( :, 8, 8 ) );
%%%%%%%%%% ターゲット初期値設定 %%%%%%%%%%
% ターゲットの姿勢・速度・角速度
SV_tc.v0 = [ zeta(12,1) zeta(13,1) 0 ]';   % ターゲット初期並進速度
SV_tc.w0 = [ 0 0 zeta(6,1) ]';   % ターゲット初期角速度
SV_tc_Q0_0 = [ 0 0 deg2rad( zeta(9,1) ) ]';   % time = 0のときの姿勢
SV_tc.Q0 = SV_tc_Q0_0 - [ 0 0 minus_time * SV_tc.w0(3,1) ]';   % time=-minus_timeのときのターゲット初期姿勢
SV_tc.A0 = rpy2dc( SV_tc.Q0 )';

% ターゲットの初期位置   tc_dammy_R0を正方形中心とする．
tc_geo_0 = [ zeta(4,1) ( POS_eL1(2,1) + POS_eL2(2,1) ) / 2 + zeta(5,1) 0 ]';
error_tcR0 = half_side_tan * zeta(10,1);   % 長方形中心がSV_tc.R0から error_R0[m] の位置にある
theta_tcR0 = deg2rad( zeta(11,1) );   % 長方形中心がSV_tc.R0から theta_R0[deg] の位置にある
SV_tc_R0_0(1,1) = tc_geo_0(1,1) - error_tcR0 * cos( theta_tcR0 + SV_tc_Q0_0(3,1) );   % time=0のときの重心位置
SV_tc_R0_0(2,1) = tc_geo_0(2,1) - error_tcR0 * sin( theta_tcR0 + SV_tc_Q0_0(3,1) );
SV_tc_R0_0(3,1) = tc_geo_0(3,1);
SV_tc.R0 = SV_tc_R0_0 - minus_time * SV_tc.v0;   % time=-minus_timeのときの初期重心位置
tc_geo(1,1) = SV_tc.R0(1,1) + error_tcR0 * cos( theta_tcR0 + SV_tc.Q0(3,1) );
tc_geo(2,1) = SV_tc.R0(2,1) + error_tcR0 * sin( theta_tcR0 + SV_tc.Q0(3,1) );
tc_geo(3,1) = SV_tc.R0(3,1);

% ターゲットの初期観測点位置
half_taikaku_chohou = sqrt( half_side_cho^2 + half_side_tan^2 );
theta_chohou = atan( half_side_cho / half_side_tan );
r_Obs1 = half_taikaku_chohou * 5/5;
r_Obs2 = half_taikaku_chohou * 5/5;
LeftUp = pi-theta_chohou; LeftDown = -pi+theta_chohou; RightUp = theta_chohou; RightDown = -theta_chohou;
kansoku1 = LeftUp; kansoku2 = LeftDown;

Obs1 = tc_geo + half_taikaku_chohou * r_Obs1 * [ cos( kansoku1 + SV_tc.Q0(3,1) ) sin( kansoku1 + SV_tc.Q0(3,1) ) 0 ]';   % 初期shikaku1と同じ点
Obs2 = tc_geo + half_taikaku_chohou * r_Obs2 * [ cos( kansoku2 + SV_tc.Q0(3,1) ) sin( kansoku2 + SV_tc.Q0(3,1) ) 0 ]';   % 初期shikaku2と同じ点
...
l_side_tan_min = r_tip + half_side_tan;   % 手先半径 + ターゲットの半短辺
l_side_cho_min = r_tip + half_side_cho;   % 手先半径 + ターゲットの半長辺
l_corner_min = r_tip + half_taikaku_chohou;   % 接触する瞬間のロボットアーム手先球中心～ターゲット中心までの距離 = 手先半径 + ターゲットの対角/2
...
% ターゲット重心 運動
fo_05_CTargetCoM_Motion = fopen( [ datpath, '/', timepath, '_', '05_CTargetCoM_Motion.dat' ], 'w' );   % ターゲットの運動に関する変数　位置，速度，加速度，姿勢，角速度，各加速度
% ターゲット手先 運動
fo_06_CTargetEE_Motion = fopen( [ datpath, '/', timepath, '_', '06_CTargetEE_Motion.dat' ], 'w' );   % ターゲット手先の運動に関する変数　四角頂点位置，接触位置(ターゲット重心座標系)，接触位置速度
% ロボット 力学
...
% ターゲット 力学
fo_08_CTarget_Kinetics = fopen( [ datpath, '/', timepath, '_', '08_CTarget_Kinetics.dat' ], 'w' );   % ターゲットの力学に関する変数　(ターゲットが受ける) 外力，外トルク
% 接触フラグ
...
% ターゲット アニメーション用
fo_16_CTarget_ForMovie = fopen( [ datpath, '/', timepath, '_', '16_CTarget_ForMovie.dat' ], 'w' );   % 重心位置，四角頂点位置，重心姿勢
% ターゲット アニメーション用
...
fprintf( fo_01_Length_etc, [ repmat( s, 1, 16 ), se ], ...   % =17コ
 'r_tip', 'side_target_tan', 'side_target_cho', 'r_motor', 'r_base', 'r_airtank', ...   % 6
...
fprintf( fo_05_CTargetCoM_Motion, [ repmat( s, 1, 18 ), se ], ...   % =19
 'time [1]', 'SV_tc.R0 [3]',u,u, 'SV_tc.v0 [3]',u,u, 'SV_tc.vd0 [3]',u,u, 'SV_tc.Q0 [3]',u,u, 'SV_tc.w0 [3]',u,u, 'SV_tc.wd0 [3]',u,u );   % 19
% ターゲット手先 運動   % ターゲット手先の運動に関する変数　四角頂点位置，接触位置(ターゲット重心座標系)，接触位置速度
fprintf( fo_06_CTargetEE_Motion, [ repmat( s, 1, 36 ), se ], ...   % =37
...
fprintf( fo_08_CTarget_Kinetics, [ repmat( s, 1, 6 ), se ], ...   % =7
 'time [1]', 'SV_tc.F0 [3]',u,u, 'SV_tc.T0 [3]',u,u );   % 7
% 接触フラグ   % 接触判定(1~4)
...
fprintf( fo_11_Estimation, [ repmat( s, 1, 31 ), se ], ....   % 32
 'time [1]', 'Obs1 [3]',u,u, 'Obs2 [3]',u,u, 'est_P [3]',u,u, 'est_V [3]',u,u, 'est_Q [3]',u,u, 'est_q [1]', 'est_W [3]',u,u, ...   % 20
             'est_geo_center [3]',u,u, 'tc_geo [3]',u,u, 'est_r1 [1]', 'est_r2 [1]', 'est_phi1 [1]', 'est_phi2 [1]', 'est_R12 [1]', 'est_Theta12 [1]' );   % 12
...
fprintf( fo_13_Momentum_ForMovie, [ repmat( s, 1, 36 ), se ], ...   % =37
 'time [1]', 'PP_d [6]',u,u,u,u,u, 'PP_tc [6]',u,u,u,u,u, 'PP [6]',u,u,u,u,u, 'PP_d_2 [6]',u,u,u,u,u, 'PP_tc_2 [6]',u,u,u,u,u, 'PP_2 [6]',u,u,u,u,u );   % 37
...
fprintf( fo_16_CTarget_ForMovie, [ repmat( s, 1, 18 ), se ], ...   % =19
 'time [1]', 'SV_tc.R0 [3]',u,u, 'shikaku1 [3]',u,u, 'shikaku2 [3]',u,u, 'shikaku3 [3]',u,u, 'shikaku4 [3]',u,u, 'SV_tc.Q0 [3]',u,u );   % 19

% dPOS
fprintf( fo_17_memo, [ repmat( s, 1, 37 ), se ], ...   % =37
 'time [1]', 'd_POSL [3]',u,u, 'dPOSR [3]',u,u,'xeL_des(1:2)',u, 'xeR_des(1:2)',u, ...
             'd_ten1',u,u, 'd_ten2',u,u, 'd_ten3',u,u, 'd_ten4',u,u, 'QL3', 'QR7', 'QL2', 'QR6', 'd_QL3', 'd_QR7', 'Phase', 'Catchtime', 'flagphase5', 't0', 'd_geo',u,u, 'delta_t', 'delta_q' ); 
...
[ time, display_time, Phase, catchtime t0]'   % 現在どのステップを計算しているかをコマンドウィンドウに出力
...
if     0 <= SV_tc.Q0(3,1) && SV_tc.Q0(3,1) < pi    % ターゲットが0～πに傾くとき
    q0 = SV_tc.Q0(3,1);
else                                               % ターゲットが-π～0のときはそのまま代入
    q0 = SV_tc.Q0(3,1) + pi;
end
...
= Chohou_FourTips_LEFT_CONTACT_v01( SV, SV_tc, d_time, r_tip, contact_flag_L1, contactflag_L1, shikaku1, shikaku2, shikaku3, shikaku4, ...
                                    l_side_tan_min, l_side_cho_min, l_corner_min, tc_geo, q0, Rg_d, POS_eL1, delta_tmp_L1, curPosAP3_tmp_L1, ...
                                    l1, l2, l3, l4, g1, g2, g3, g4, cof, kkk, ccc, half_side_tan, half_side_cho );
...
= Chohou_FourTips_LEFT_CONTACT_v01( SV, SV_tc, d_time, r_tip, contact_flag_L2, contactflag_L2, shikaku1, shikaku2, shikaku3, shikaku4, ...
                                    l_side_tan_min, l_side_cho_min, l_corner_min, tc_geo, q0, Rg_d, POS_eL2, delta_tmp_L2, curPosAP3_tmp_L2, ...
                                    l1, l2, l3, l4, g1, g2, g3, g4, cof, kkk, ccc, half_side_tan, half_side_cho );
%%%%%%%%%% 右側3,4 %%%%%%%%%%
...
= Chohou_FourTips_RIGHT_CONTACT_v01( SV, SV_tc, d_time, r_tip, contact_flag_R1, contactflag_R1, shikaku1, shikaku2, shikaku3, shikaku4, ...
                                     l_side_tan_min, l_side_cho_min, l_corner_min, tc_geo, q0, Rg_d, POS_eR1, delta_tmp_R1, curPosAP3_tmp_R1, ...
                                     l1, l2, l3, l4, g1, g2, g3, g4, cof, kkk, ccc, half_side_tan, half_side_cho );
...
= Chohou_FourTips_RIGHT_CONTACT_v01( SV, SV_tc, d_time, r_tip, contact_flag_R2, contactflag_R2, shikaku1, shikaku2, shikaku3, shikaku4, ...
                                     l_side_tan_min, l_side_cho_min, l_corner_min, tc_geo, q0, Rg_d, POS_eR2, delta_tmp_R2, curPosAP3_tmp_R2, ...
                                     l1, l2, l3, l4, g1, g2, g3, g4, cof, kkk, ccc, half_side_tan, half_side_cho );
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
SV_tc.F0 = -( FR_L1 + FR_L2 + FR_R1 + FR_R2 );      % ターゲットが受ける力(左右からの合力) F3_1(左手,慣性右方向) + F3_2(右手,慣性左方向)
SV_tc.T0 = T_t3_L1 + T_t3_L2 + T_t3_R1 + T_t3_R2;    % ターゲットが受けるトルク(左右からの合トルク)
FR_L = FR_L1 + FR_L2;
FR_R = FR_R1 + FR_R2;
T_d3_L = T_d3_L1 + T_d3_L2;
T_d3_R = T_d3_R1 + T_d3_R2;
SV.Fe(:,4) = FR_L;    % ロボット(左)が受ける力 (左向きにする)
SV.Te(:,4) = T_d3_L;   % ロボット(左)が受けるトルク
SV.Fe(:,8) = FR_R;    % ロボット(右)が受ける力 (右向きにする)
SV.Te(:,8) = T_d3_R;   % ロボット(右)が受けるトルク
...
est_Q_tmp = est_Q;
% 更新   % ここは未知情報を代入してよい
Obs1 = tc_geo + r_Obs1 * [ cos( kansoku1 + SV_tc.Q0(3,1) ), sin( kansoku1 + SV_tc.Q0(3,1) ), 0 ]';% + [ randi(10,1) * delta, randi(10,1) * delta, 0 ]';   % 初期shikaku1と同じ点を観測
Obs2 = tc_geo + r_Obs2 * [ cos( kansoku2 + SV_tc.Q0(3,1) ), sin( kansoku2 + SV_tc.Q0(3,1) ), 0 ]';% + [ randi(10,1) * delta, randi(10,1) * delta, 0 ]';   % 初期shikaku2と同じ点を観測
...
= Chohou_Estimation_v01( d_time, time, Obs1, Obs2, Obs1_tmp, Obs2_tmp, NorTraj_12_tmp, est_P_tmp, est_Q_tmp, contactflag_L1, contactflag_L2, contactflag_R1, contactflag_R2, ...
                         est_phi1, est_phi2, est_r1, est_r2, est_P, est_V, est_erP, NorTraj_12, est_Theta12, half_taikaku_chohou, theta_chohou );
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% 一般化ヤコビ行列計算 手先
    JL = calc_gj( LP, SV, num_eL );   % 一般化ヤコビ行列(左手)
    JR = calc_gj( LP, SV, num_eR );   % 一般化ヤコビ行列(右手)
...
% ヤコビ行列計算 手首
    ...
    Hs = HH(1:6, 1:6);
      ...
      JmL = calc_je( LP, SV, [ 1 2 3 ] );
      [ pL, oL ] = f_kin_j( LP, SV, [ 1 2 3 ] );   % 手先だからcalc_gjでも
      JsL = [   eye( 3, 3 ) -tilde( pL(:,3)-SV.R0 );
              zeros( 3, 3 )               eye( 3, 3 ) ];
      HmL = HH(1:6, 7:6 + LP.num_q);
    GJL = JmL - JsL * Hs \ HmL;
...
%       QL_2 = dc2rpy( oL(:,end-5:end-3)' );
      ...
      JmR = calc_je( LP, SV, [ 5 6 7 ] );
      [ pR, oR ] = f_kin_j( LP, SV, [ 5 6 7 ] );
      JsR = [   eye( 3, 3 ) -tilde( pR(:,3)-SV.R0 );
              zeros( 3, 3 )               eye( 3, 3 ) ];
      HmR = HH( 1:6, 7:6 + LP.num_q );
    GJR = JmR - JsR * Hs \ HmR;
...
%%%%%%%%%%%%%%%%%%%% 目標手先速度の計算 %%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
[ xe_des, ve_des, ae_des, POS_j4_tmp, POS_j8_tmp, d_POS_j4, d_POS_j8, VEL_j4, VEL_j8, ...
...
= Chohou_DesiredHandMotion_v01( d_time, time, minus_time, SV, Obs1, Obs2, est_P, est_Q, est_V, est_W, est_geo, ...
...
qdd = Je_3_G * ...
      ( ae_des ...
      + Kd_ichi * ( [ ve_des(1:3,1)', zeros(1,3), ve_des(7:9,1)', zeros(1,3) ]' - [ VEL_j4', zeros(1,4), VEL_j8', zeros(1,4) ]' ) ...   % 速度
      + Kp_ichi * ( [ xe_des(1:3,1)', zeros(1,3), xe_des(7:9,1)', zeros(1,3) ]' - [ POS_j4', zeros(1,4), POS_j8', zeros(1,4) ]' ) ...   % 位置
      + Kd_kaku * ( [ zeros(1,5), ve_des(6,1)', zeros(1,5), ve_des(12,1)' ]' - [ zeros(1,5), qd_joints(3,1), zeros(1,5), qd_joints(7,1) ]' ) ...   % 角速度
      + Kp_kaku * ( [ zeros(1,5), xe_des(6,1)', zeros(1,5), xe_des(12,1)' ]' - [ zeros(1,5),  q_joints(3,1), zeros(1,5),  q_joints(7,1) ]' ) ...   % 姿勢
      + Jd_3_G * qd_joints );
qdd_2 = Je_3_G * ...
      ( ae_des ...
      + Kd_kaku * ( [ zeros(1,5), ve_des(6,1)', zeros(1,5), ve_des(12,1)' ]' - [ zeros(1,5), qd_joints(3,1), zeros(1,5), qd_joints(7,1) ]' ) ...   % 角速度
...
if     Phase == 4 && flagphase5 ~= 1    % time >= round( (t0+delta_t)/d_time )*d_time   % flag_kotei == 1
    SV.tau = H_asuta * qdd + C_asuta - JL' * Fh_L - JR' * Fh_R ...   % 位置制御 
             + Kd_wrist * ( zeros(8,1) - qd_wrist ) + Kp_wrist * ( zeros(8,1) - q_wrist );   % 手首pd制御

elseif Phase == 5 || flagphase5 == 1
     if catchtime > ( hokakutime - d_time/2 )  &&  catchtime < ( hokakutime + d_time/2 )   % == でやると分岐はいらない場合があるため
        flagphase5 = 1;
        q_joints_kotei = q_joints;   % アーム固定のために姿勢保存
     end
% KL = kakudo( [ POS_j3' 0 ]', [ ( 2 * POS_j3 - POS_j2 )' 0 ]', [ POS_j7' 0 ]' );
% KR = kakudo( [ POS_j7' 0 ]', [ ( 2 * POS_j7 - POS_j6 )' 0 ]', [ POS_j3' 0 ]' );
KL = kakudo( [ POS_j3' 0 ]', [ ( 2 * POS_j3 - POS_j2 )' 0 ]', cen_L );
KR = kakudo( [ POS_j7' 0 ]', [ ( 2 * POS_j7 - POS_j6 )' 0 ]', cen_R );
    SV.tau = - JL' * Fh_L - JR' * Fh_R ... H_asuta * qdd_2 + C_asuta - JL' * Fh_L - JR' * Fh_R ... 
...             + 1 * ( zeros(8,1) - [ qd_joints(1:3)', 0, qd_joints(5:7)', 0 ]' ) ...
...             + 3 * ( [ q_joints_kotei(1:3)', 0, q_joints_kotei(5:7)', 0 ]' - [ q_joints(1:3)', 0, q_joints(5:7)', 0 ]' ) ...
             + 1 * ( zeros(8,1) - [ qd_joints(1:3)', 0, qd_joints(5:7)', 0 ]' ) ...   % 姿勢制御
...
%   end
    ...
    SV.tau = H_asuta * qdd + C_asuta - JL' * Fh_L - JR' * Fh_R ...
...
SV = calc_pos( LP, SV );
% 全関節位置姿勢の計算
[ POS_j_L, ORI_j_L ] = f_kin_j( LP, SV, jointsL );   % 左手1
[ POS_j_R, ORI_j_R ] = f_kin_j( LP, SV, jointsR );   % 右手3
% 各関節の位置 x,y
POS_j1 = POS_j_L( 1:2, 1 );   % 関節1の位置代入
POS_j2 = POS_j_L( 1:2, 2 );
POS_j3 = POS_j_L( 1:2, 3 );
...
SV.Q0 = dc2rpy( SV.A0' );       
% 角度をradからdegへ変換
Q0_deg_d = rad2deg( SV.Q0 );   % ベースの姿勢[deg]
...
q_deg = rad2deg( SV.q );   % 関節の姿勢[deg]
% ターゲット姿勢計算
SV_tc.Q0 = dc2rpy( SV_tc.A0' );   % ターゲットの方向余弦行列を角度に変換
Q0_deg_t = rad2deg( SV_tc.Q0 );
...
% ロボット
  ...
  PP_d =  Hb_d * [ SV.v0' SV.w0' ]' + Hbm_d * SV.qd;  % ベース重心周り

% ターゲット
  HH_tc = calc_hh( LP_tc, SV_tc );
  Hb_tc = HH_tc( 1:6, 1:6 );
  PP_tc = Hb_tc * [ SV_tc.v0' SV_tc.w0' ]';

% 系全体
%   R_d = SV_d.R0 - Rg_d;
%   R_ts = SV_ts.R0 - Rg_d;
  R = SV_tc.R0 - SV.R0;
  PP( 1:3, 1 ) = PP_d( 1:3, 1 ) + PP_tc( 1:3, 1 );
  PP( 4:5, 1 ) = zeros( 2, 1 );
  PP( 6, 1 ) = PP_d( 6, 1 ) + PP_tc( 6, 1 ) + LP_tc.mass * ( R( 1, 1 ) * SV_tc.v0( 2, 1 ) - R( 2, 1 ) * SV_tc.v0( 1, 1 ) );
...
%   PP_d_2 = [ P_d' L_d' ]';
% % ターゲット重心位置の歪対称行列
%   target_tilde = tilde( SV_tc.R0 );
% % ターゲットの並進運動量P_t, 角運動量L_t
%   P_tc = LP_tc.m0 * SV_tc.v0;
%   L_tc = LP_tc.inertia0 * SV_tc.w0 + target_tilde * P_tc;
%   PP_tc_2 = [ P_tc' L_tc' ]';
% % 系全体の運動量
%   PP_2 = PP_d_2 + PP_tc_2;
...
fprintf( fo_01_Length_etc, [ repmat( g, 1, 16 ), ge ], ...   % =17コ
         r_tip, half_side_tan, half_side_cho, r_motor, r_base, r_airtank, ...   % 6
         base_tate, base_yoko, base_jushin_teihen', base_arm_yoko_1, base_arm_yoko_2, base_arm_tate_1, base_arm_tate_2, base_airtank_yoko, base_airtank_tate, L_hiraki, theta_hiraki );   % 11
end
% ロボット重心 運動   % ロボット重心の運動に関する変数　位置，速度，加速度，姿勢，角速度，各加速度
fprintf( fo_02_RobotCoM_Motion, [ repmat( g, 1, 18 ), ge ], ...   % =19
         display_time, SV.R0, SV.v0, SV.vd0, SV.Q0, SV.w0, SV.wd0 );   % 19
% ロボット手先 運動   % ロボット手先の運動に関する変数　位置，姿勢，接触位置(ロボット重心座標系)
...
fprintf( fo_04_RobotLink_Motion, [ repmat( g, 1, 36 ), ge ], ...   % =37
         ...
         SV.q, veL_des, veR_des );   % 20
% ターゲット重心 運動   % ターゲットの運動に関する変数　位置，速度，加速度，姿勢，角速度，各加速度
fprintf( fo_05_CTargetCoM_Motion, [ repmat( g, 1, 18 ), ge ], ...   % =19
         display_time, SV_tc.R0, SV_tc.v0, SV_tc.vd0, SV_tc.Q0, SV_tc.w0, SV_tc.wd0 );   % 19
% ターゲット手先 運動   % ターゲット手先の運動に関する変数　四角頂点位置，接触位置(ターゲット重心座標系)，接触位置速度
fprintf( fo_06_CTargetEE_Motion, [ repmat( g, 1, 36 ), ge ], ...   % =37
...
fprintf( fo_07_Robot_Kinetics, [ repmat( g, 1, 56 ), ge ], ...   % =57
         ...
         SV.tau );   % 8
% ターゲット 力学      % ターゲットの力学に関する変数　(ターゲットが受ける) 外力，外トルク
fprintf( fo_08_CTarget_Kinetics, [ repmat( g, 1, 6 ), ge ], ...   % =7
         display_time, SV_tc.F0, SV_tc.T0 );   % 7
% 接触フラグ   % 接触判定(1~4)
...
fprintf( fo_11_Estimation, [ repmat( g, 1, 31 ), ge ], ....   % 32
         ...
         est_geo, tc_geo, est_r1, est_r2, est_phi1, est_phi2, est_R12, est_Theta12 );   % 12
% めり込み   % l_x_H(1~4), delta_H(1~4), deltaVel_H(1~4)
...
fprintf( fo_13_Momentum_ForMovie, [ repmat( g, 1, 36 ), ge ], ...   % =37
         display_time, PP_d, PP_tc, PP, ...   % 19
         PP_d_2, PP_tc_2, PP_2 );   % 18
% 力 アニメーション用   % 法線単位ベクトル，接線単位ベクトル，法線力，接線力
...
fprintf( fo_15_Robot_ForMovie, [ repmat( g, 1, 46 ), ge ], ...   % =47
         ...
         SV.Q0 );   % 3
% ターゲット アニメーション用    % 重心位置，関節位置，手先位置，重心姿勢
fprintf( fo_16_CTarget_ForMovie, [ repmat( g, 1, 18 ), ge ], ...   % =19
         display_time, SV_tc.R0, shikaku1, shikaku2, shikaku3, shikaku4, SV_tc.Q0 );   % 19

% d_POS
fprintf( fo_17_memo, [ repmat( g, 1, 37 ), ge ], ...   % =37
         display_time, d_POS_j4, d_POS_j8, xe_des(1:2), xe_des(7:8), d_ten1, d_ten2, d_ten3, d_ten4, QL_3(3,1), QR_7(3,1), QL_2(3,1), QR_6(3,1), d_QL_3, d_QR_7, Phase, catchtime, flagphase5, t0, d_geo, delta_t, delta_q );   % 38
...
% ターゲット アニメーション用
mat_16 = dlmread( [ datpath, '/', timepath, '_', '16_CTarget_ForMovie.dat' ], '\t', 1, 0 ); 
...
half_side_tan = mat_01(2);   % ターゲット半短辺
half_side_cho = mat_01(3);   % ターゲット半長辺
...
% ターゲット
pos_tc =  mat_16(:,2:3);  % ターゲット重心位置x,y
pos_tc_geo =  mat_11(:,21:23);  % ターゲット幾何中心位置x,y
...
%%%%%%%%%% リンク頂点設定 %%%%%%%%%%
L_hiraki = tekubiyubisaki; b = sin(hirakikaku);
%%%%% 左腕 %%%%%
arm_L_x = [ pos_j1(:,1) pos_j2(:,1) pos_j3(:,1) pos_j4(:,1) ]; arm_L_y = [ pos_j1(:,2) pos_j2(:,2) pos_j3(:,2) pos_j4(:,2) ];   % 関節1,2,3,4の座標設定
hand_L_x = [ pos_eL1(:,1) pos_j4(:,1)-L_hiraki*b*cos(rad_eL1(:,1)) pos_j4(:,1)+L_hiraki*b*cos(rad_eL1(:,1)) pos_eL2(:,1) ];   % 手先のx座標設定
hand_L_y = [ pos_eL1(:,2) pos_j4(:,2)-L_hiraki*b*sin(rad_eL1(:,1)) pos_j4(:,2)+L_hiraki*b*sin(rad_eL1(:,1)) pos_eL2(:,2) ];   % y座標
%%%%% 右腕 %%%%%
arm_R_x = [ pos_j5(:,1) pos_j6(:,1) pos_j7(:,1) pos_j8(:,1) ]; arm_R_y = [ pos_j5(:,2) pos_j6(:,2) pos_j7(:,2) pos_j8(:,2) ];   % 手先の座標設定
hand_R_x = [ pos_eR1(:,1) pos_j8(:,1)-L_hiraki*b*cos(rad_eR1(:,1)) pos_j8(:,1)+L_hiraki*b*cos(rad_eR1(:,1)) pos_eR2(:,1) ];   % 手先のx座標設定
hand_R_y = [ pos_eR1(:,2) pos_j8(:,2)-L_hiraki*b*sin(rad_eR1(:,1)) pos_j8(:,2)+L_hiraki*b*sin(rad_eR1(:,1)) pos_eR2(:,2) ];   % y座標
...
if simplemode == 1 && kakudaimode == 1
  if gridmode == 1
     moviename = [ movpath, '/', timepath, '_DualArm_TargetChohou_', 'FrameRate', num2str(framerate), '_MovSimpleKakudaiGrid.avi' ];
  else
     moviename = [ movpath, '/', timepath, '_DualArm_TargetChohou_', 'FrameRate', num2str(framerate), '_MovSimpleKakudai.avi' ];
  end
else
     moviename = [ movpath, '/', timepath, '_DualArm_TargetChohou_', 'FrameRate', num2str(framerate), '_Mov.avi' ];
end
...
% ロボット
    ...
    tg_cross_1 = fill( [ pos_tc(1,1), ( pos_tc(1,1) + r_base * cos( ang_tr(1,1) + for_pi_half ) ),        pos_tc(1,1), ( pos_tc(1,1) + r_base * cos( ang_tr(1,1) + for_pi_half + pi ) ),     pos_tc(1,1) ], ...
                       [ pos_tc(1,2), ( pos_tc(1,2) + r_base * sin( ang_tr(1,1) + for_pi_half ) ),        pos_tc(1,2), ( pos_tc(1,2) + r_base * sin( ang_tr(1,1) + for_pi_half + pi ) ),     pos_tc(1,2) ], ...
                         kuro, 'LineWidth', sen1 );
    tg_cross_2 = fill( [ pos_tc(1,1), ( pos_tc(1,1) + r_base * cos( ang_tr(1,1) + for_pi_half + pi/2 ) ), pos_tc(1,1), ( pos_tc(1,1) + r_base * cos( ang_tr(1,1) + for_pi_half + 3*pi/2 ) ), pos_tc(1,1) ], ...
                       [ pos_tc(1,2), ( pos_tc(1,2) + r_base * sin( ang_tr(1,1) + for_pi_half + pi/2 ) ), pos_tc(1,2), ( pos_tc(1,2) + r_base * sin( ang_tr(1,1) + for_pi_half + 3*pi/2 ) ), pos_tc(1,2) ], ...
                         shiro, 'LineWidth', sen1 );

if simplemode == 0
    % 推定情報
    obs1 = fill( obs1_tr(1,1) + r0s, obs1_tr(1,2) + r0c, 'g', 'LineWidth', sen1 );
    obs2 = fill( obs2_tr(1,1) + r0s, obs2_tr(1,2) + r0c, kuro, 'LineWidth', sen1 );
    d_en_geo = fill( d_geo(1,1) + r0c, d_geo(1,2) + r0s, 'g-', 'LineWidth', sen1 );
    tg_en_geo = fill( pos_tc_geo(1,1) + r0c*3/2, pos_tc_geo(1,2) + r0s*3/2, 'w-', 'LineWidth', sen1 );
...
saveas( figure(n_figure), [ movpath, '/', timepath, '_DualArm_TargetChohou_1stFig.fig' ] );
saveas( figure(n_figure), [ movpath, '/', timepath, '_DualArm_TargetChohou_1stFig.png' ] );

%%%%%%%%%% 図を更新 %%%%%%%%%%
i = 1;
while i <= length( mvtime(:,1) )

% if mvtime(i) > mvtime(end); %10 
% break
% end

    if rem( mvtime(i,1), movie_d_time ) == 0   % mvtime(i)をmovie_d_timeで除算した後の剰余が0のとき(割り切れるとき)
...
if simplemode == 0
     ...
     % ターゲット
       ...
       set( tg_cross_1, 'XData', [ pos_tc(i,1), ( pos_tc(i,1) + r_base * cos( ang_tr(i,1) + for_pi_half ) ),        pos_tc(i,1), ( pos_tc(i,1) + r_base * cos( ang_tr(i,1) + for_pi_half + pi ) ),     pos_tc(i,1) ], ...
                        'YData', [ pos_tc(i,2), ( pos_tc(i,2) + r_base * sin( ang_tr(i,1) + for_pi_half ) ),        pos_tc(i,2), ( pos_tc(i,2) + r_base * sin( ang_tr(i,1) + for_pi_half + pi ) ),     pos_tc(i,2) ] );
       set( tg_cross_2, 'XData', [ pos_tc(i,1), ( pos_tc(i,1) + r_base * cos( ang_tr(i,1) + for_pi_half + pi/2 ) ), pos_tc(i,1), ( pos_tc(i,1) + r_base * cos( ang_tr(i,1) + for_pi_half + 3*pi/2 ) ), pos_tc(i,1) ], ...
                        'YData', [ pos_tc(i,2), ( pos_tc(i,2) + r_base * sin( ang_tr(i,1) + for_pi_half + pi/2 ) ), pos_tc(i,2), ( pos_tc(i,2) + r_base * sin( ang_tr(i,1) + for_pi_half + 3*pi/2 ) ), pos_tc(i,2) ] );

if simplemode == 0
     % 推定情報
       set( obs1, 'XData', obs1_tr(i,1) + r0s, 'YData', obs1_tr(i,2) + r0c );
       set( obs2, 'XData', obs2_tr(i,1) + r0s, 'YData', obs2_tr(i,2) + r0c );
       ...
       set( tg_en_geo, 'XData', pos_tc_geo(i,1) + r0c*3/2, 'YData', pos_tc_geo(i,2) + r0s*3/2 );
...
%{
    if rem( mvtime(i), movie_d_time * 5 ) == 0   % mvtime(i)をmovie_d_timeで除算した後の剰余が0のとき(割り切れるとき)
        ...
        figure(n_figure)
        title( sprintf( 'Time = %0.3f [s]', mvtime(i,1) ), 'FontName', 'Times New Roman', 'FontSize', FST );
        saveas( figure(n_figure), [ movpath, '\', timepath, '_DualArm_TargetChohou_TrackingContact_', num2str(mvtime(i)*100), 'Figure.fig' ] );
        saveas( figure(n_figure), [ movpath, '\', timepath, '_DualArm_TargetChohou_TrackingContact_', num2str(mvtime(i)*100), 'Figure.png' ] );
    end
%}
end
...
saveas( figure(n_figure), [ movpath, '/', timepath, '_DualArm_TargetChohou_LastFig.fig' ] );
saveas( figure(n_figure), [ movpath, '/', timepath, '_DualArm_TargetChohou_LastFig.png' ] );
% n_figure = n_figure + 1;
