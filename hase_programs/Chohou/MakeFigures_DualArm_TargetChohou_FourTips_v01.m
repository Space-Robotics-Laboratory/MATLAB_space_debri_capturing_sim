...
path = 'dt=0.001,et=20,Xtc=0,Ytc=0,Vxtc=0,Vytc=0,THtc=0,Wtc=1,ErL=0.25,ErTH=45';

'2019-0127-205005_dt=0.001,et=20,Xtc=0,Ytc=0,Vxtc=0,Vytc=0,THtc=0,Wtc=1,ErL=0.25,ErTH=45';

% 時間指定
timepath = [ day, '-', '205005' ] ;
% パス指定
datepath = [ 'C:/Users/falsi/Desktop/Matlab/BackUp/', day, '/', timepath, '_' ];
datepath = [ 'C:\Users\falsi\Desktop\最終審査\figs\v2\SimC/', timepath, '_' ];
...
% ターゲット重心 運動
mat_05 = dlmread( [ datpath, '/', timepath, '_', '05_CTargetCoM_Motion.dat' ], '\t', 1, 0 );
% ターゲット重心 運動
mat_06 = dlmread( [ datpath, '/', timepath, '_', '06_CTargetEE_Motion.dat' ], '\t', 1, 0 );
% ロボット 力学
mat_07 = dlmread( [ datpath, '/', timepath, '_', '07_Robot_Kinetics.dat' ], '\t', 1, 0 );
% ターゲット 力学
mat_08 = dlmread( [ datpath, '/', timepath, '_', '08_CTarget_Kinetics.dat' ], '\t', 1, 0 );
% 接触フラグ
...
% phi = theta_t(i,1);
% phi = deg2rad(f6(i,1));
phi = theta_t(i,1) - theta_EE2 - atan( cho / tan );
...
Maxa(i,1) = ( d + sqrt(tan^2+cho^2) ) * cos( phi ); abs( cos( phi ) );
